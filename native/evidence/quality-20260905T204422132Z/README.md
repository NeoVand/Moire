...
Reproduce: `node native/tools/compare_mrq.mjs`. Explicit `--raw`, `--tsr`, `--analytic` report paths and a fresh `--out` directory are also accepted. This performs CPU image analysis only and needs no Unreal installation or generated assets. PNGs resolve from each report directory’s `frames/` folder; the exact preparation JSON resolves from the sibling `mrq-prepare-20260905T203352.265013Z/preparation.json`. Captured absolute paths remain provenance only. Full per-pixel data, capture hashes, configuration, and actual queried cvars are in [report.json](report.json).

Portability was also tested with reports, PNGs, and the archived preparation copied to a temporary tree. Both captured absolute input paths were redirected to nonexistent locations; the runner still exited zero and every per-pixel value and summary matched exactly. See [relocation-proof.json](relocation-proof.json).
