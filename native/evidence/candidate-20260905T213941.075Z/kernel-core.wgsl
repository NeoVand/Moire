...
fn quadRegion(a0: f32, g: vec2f, H: vec3f, S: f32) -> f32 {
  ...
  for (var seg: i32 = 0; seg < 3; seg++) {
      ...
      WORK += 1.0;
    {
      ...
      acc += 0.02715245941 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = 0.9445750231;
      var t: f32 = mid + half * x;
      var jac: f32 = half;
      if (mapA && mapB) { t = mid + half * sin(0.5 * PI * x); jac = half * 0.5 * PI * cos(0.5 * PI * x); }
      else if (mapA) { let sN: f32 = 0.5 * (x + 1.0); t = pa + dz * sN * sN; jac = dz * sN; }
      ...
      let phi: f32 = 0.3989422804014327 * exp(-0.5 * t * t / S) / sig;
      acc += 0.06225352394 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = 0.8656312024;
      var t: f32 = mid + half * x;
      var jac: f32 = half;
      if (mapA && mapB) { t = mid + half * sin(0.5 * PI * x); jac = half * 0.5 * PI * cos(0.5 * PI * x); }
      else if (mapA) { let sN: f32 = 0.5 * (x + 1.0); t = pa + dz * sN * sN; jac = dz * sN; }
      ...
      let phi: f32 = 0.3989422804014327 * exp(-0.5 * t * t / S) / sig;
      acc += 0.09515851168 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = 0.7554044084;
      var t: f32 = mid + half * x;
      var jac: f32 = half;
      if (mapA && mapB) { t = mid + half * sin(0.5 * PI * x); jac = half * 0.5 * PI * cos(0.5 * PI * x); }
      else if (mapA) { let sN: f32 = 0.5 * (x + 1.0); t = pa + dz * sN * sN; jac = dz * sN; }
      ...
      let phi: f32 = 0.3989422804014327 * exp(-0.5 * t * t / S) / sig;
      acc += 0.1246289713 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = 0.6178762444;
      var t: f32 = mid + half * x;
      var jac: f32 = half;
      if (mapA && mapB) { t = mid + half * sin(0.5 * PI * x); jac = half * 0.5 * PI * cos(0.5 * PI * x); }
      else if (mapA) { let sN: f32 = 0.5 * (x + 1.0); t = pa + dz * sN * sN; jac = dz * sN; }
      ...
      let phi: f32 = 0.3989422804014327 * exp(-0.5 * t * t / S) / sig;
      acc += 0.1495959888 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = 0.4580167777;
      var t: f32 = mid + half * x;
      var jac: f32 = half;
      if (mapA && mapB) { t = mid + half * sin(0.5 * PI * x); jac = half * 0.5 * PI * cos(0.5 * PI * x); }
      else if (mapA) { let sN: f32 = 0.5 * (x + 1.0); t = pa + dz * sN * sN; jac = dz * sN; }
      ...
      let phi: f32 = 0.3989422804014327 * exp(-0.5 * t * t / S) / sig;
      acc += 0.1691565194 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = 0.2816035508;
      var t: f32 = mid + half * x;
      var jac: f32 = half;
      if (mapA && mapB) { t = mid + half * sin(0.5 * PI * x); jac = half * 0.5 * PI * cos(0.5 * PI * x); }
      else if (mapA) { let sN: f32 = 0.5 * (x + 1.0); t = pa + dz * sN * sN; jac = dz * sN; }
      ...
      let phi: f32 = 0.3989422804014327 * exp(-0.5 * t * t / S) / sig;
      acc += 0.1826034150 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = 0.09501250984;
      var t: f32 = mid + half * x;
      var jac: f32 = half;
      if (mapA && mapB) { t = mid + half * sin(0.5 * PI * x); jac = half * 0.5 * PI * cos(0.5 * PI * x); }
      else if (mapA) { let sN: f32 = 0.5 * (x + 1.0); t = pa + dz * sN * sN; jac = dz * sN; }
      ...
      let phi: f32 = 0.3989422804014327 * exp(-0.5 * t * t / S) / sig;
      acc += 0.1894506105 * jac * phi * innerProb(lin, gin + lmix * t, a0 + gout * t + 0.5 * lout * t * t, sig);
    }
    {
      let x: f32 = -0.09501250984;
      var t: f32 = mid + half * x;
...
fn bvnu(h: f32, k: f32, r: f32) -> f32 {
  ...
  var bvn: f32 = 0.0;
  if (abs(r) < 0.3) {
  { let sn: f32 = sin(0.9662347571 * asr); bvn += 0.1713244924 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.8306046932 * asr); bvn += 0.3607615730 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.6193095930 * asr); bvn += 0.4679139346 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.3806904070 * asr); bvn += 0.4679139346 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.1693953068 * asr); bvn += 0.3607615730 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.03376524290 * asr); bvn += 0.1713244924 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  } else if (abs(r) < 0.75) {
  { let sn: f32 = sin(0.9801449282 * asr); bvn += 0.1012285363 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.8983332387 * asr); bvn += 0.2223810345 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.7627662050 * asr); bvn += 0.3137066459 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.5917173212 * asr); bvn += 0.3626837834 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.4082826788 * asr); bvn += 0.3626837834 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.2372337950 * asr); bvn += 0.3137066459 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.1016667613 * asr); bvn += 0.2223810345 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.01985507175 * asr); bvn += 0.1012285363 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  } else {
  { let sn: f32 = sin(0.9907803171 * asr); bvn += 0.04717533639 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.9520586282 * asr); bvn += 0.1069393260 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.8849513371 * asr); bvn += 0.1600783285 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.7936589771 * asr); bvn += 0.2031674267 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.6839157495 * asr); bvn += 0.2334925365 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.5626167043 * asr); bvn += 0.2491470458 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.4373832957 * asr); bvn += 0.2491470458 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.3160842505 * asr); bvn += 0.2334925365 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.2063410229 * asr); bvn += 0.2031674267 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.1150486629 * asr); bvn += 0.1600783285 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.04794137181 * asr); bvn += 0.1069393260 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { let sn: f32 = sin(0.009219682877 * asr); bvn += 0.04717533639 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  }
...
fn bvnuHigh(h: f32, k: f32, r: f32) -> f32 {
  ...
  { let xx: f32 = mid + hw * 0.9894009350; acc += 0.02715245941 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * 0.9445750231; acc += 0.06225352394 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * 0.8656312024; acc += 0.09515851168 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * 0.7554044084; acc += 0.1246289713 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * 0.6178762444; acc += 0.1495959888 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * 0.4580167777; acc += 0.1691565194 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * 0.2816035508; acc += 0.1826034150 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * 0.09501250984; acc += 0.1894506105 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * -0.09501250984; acc += 0.1894506105 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * -0.2816035508; acc += 0.1826034150 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * -0.4580167777; acc += 0.1691565194 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
  { let xx: f32 = mid + hw * -0.6178762444; acc += 0.1495959888 * hw * 0.3989422804014327 * exp(-0.5 * xx * xx) * Phi(-(k - r * xx) / s); }
