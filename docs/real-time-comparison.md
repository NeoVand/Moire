...
The next native milestone is a moving main-game view with camera-derived analytic inputs, synchronized cuts and identical trajectories across methods, followed by one-method-at-a-time whole-frame timing at 1080p. This requires replacing the analytic material's fixed camera rows and verifying the native capture route. Unsupported kernel regimes must also be counted and made visible during validation. More still-image reference samples alone do not complete the user's real-time comparison.
