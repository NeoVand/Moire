...
## Portable shader and bounded iteration

The portable refactor `1d6a1a0` reproduces all 100 direct fixture outputs, all 120 material probe fields, and deterministic live image hashes exactly on this GPU. Its [validation manifest](compare-evidence/portable-2026-09-05T20-09-37.745Z/validation-manifest.json) and [numerical comparison](compare-evidence/portable-2026-09-05T20-09-37.745Z/numerical-comparison.json) preserve the unchanged source hashes. That browser result did not imply HLSL compilation: the first real DXC call found two missing forward declarations. Claude corrected declaration order and namespaced constant names; the [real compiler utilities](../native/tools/README.md) now compile the shared source and actual native material bodies.

Read-only float32 review also found edge/cell counters that could stop advancing at large integer phases. The diagnostic [watchdog](../tests/compare/watchdog.mjs) instruments every loop with a shared 16,384-iteration limit, audits that no loop was missed, and proves the limiter works with a nested infinite-loop/continue control. It never dispatches the old kernels without that guard and is not used for timing. The [three-version GPU report](compare-evidence/watchdog-2026-09-05T20-20-38.605Z.json) confirms historical `1d6a1a0` exhausts on both large-phase cases; `50a85b1` and `1612267` return within the guard. Interrupted values are not image estimates.

A third, near-collinear fixture was initially predicted to hang from a per-operation CPU float32 model. It did **not** hang on this GPU: generated arithmetic reached the old recipe cap instead. The [initial failed expectation](compare-evidence/watchdog-initial-expectation-failure.json) is retained. The corrected runner observes that historical case rather than asserting a hardware-universal hang. The cap still exposed a semantic issue: accepted-term counts do not bound rejected candidates or guarantee complete integration.

`1612267` bounds visited candidates and reports declined/exhausted work as regime 4. Its near-collinear checker and circle fixtures return in 19 and 2 instrumented iterations respectively, both tagged regime 4. The large-phase circle returns a tagged incomplete value; the material adapters currently display the value component, so unsupported-pixel diagnostics remain outstanding. The watchdog establishes bounded work for its fixtures, not numerical correctness, an all-input proof, or shader speed.

The new [100-case direct gate](compare-evidence/bounded-homography-2026-09-05T20-20-27.471Z.json), [120-probe float material gate](compare-evidence/bounded-materials-2026-09-05T20-20-28.274Z.json) and [live controls](compare-evidence/bounded-live-2026-09-05T20-20-30.917Z.json) pass with stable source hashes. Shared homography RMS is `0.000453654 / 0.000247884 / 0.000516810`; the third pose changes slightly from the prior version while all byte captures agree. These remain selected-pixel Gaussian-reference measurements, limited by the same reference convergence.

The native host now imports the shared HLSL, supplies the same fixed-pose homographies and actual viewport coordinates, and generates source/analytic maps plus camera-cut sequences. Its [material compiler and NullRHI evidence](../native/tools/README.md) verify language compilation and editor bindings. [Controlled native captures](../native/tools/capture-native.md) use the actual game viewport and Unreal TSR, with counted sequence warm-up and fresh processes. Captured pixels, Metal material behavior, output calibration, history state and uncontended whole-frame performance must be verified before using those images as a native quality or speed comparison.

The first native raw attempt initialized Metal SM6 and logged the requested AA0/100% settings, completed shader compilation and sequence frames 0–64, then failed to deliver its PNG. The [failure diagnosis](../native/evidence/capture-20260905T202331.609258Z-raw-Glide0/diagnosis.json) distinguishes observed missing output from the inferred Mac offscreen/backbuffer cause. Only the owned process was stopped. No TSR/analytic capture or native-image result was produced by that attempt.

## First native Unreal images

The [native comparison gallery](../native/comparison.html) now shows actual Unreal 5.8.2 Metal renders of the same Glide0 pose: unfiltered point shading, native Unreal TSR, and the frozen `1612267` analytic HLSL. The [batch manifest](../native/evidence/mrq-prepare-20260905T203352.265013Z/capture-batch.json) verifies common source/config hashes and camera. All images are 640×360, with one spatial sample, one temporal sample, 64 discarded render warm-ups and frame 64 saved. TSR uses 100% input/output resolution and a 200% history; its own jitter remains active.

The successful route is [Movie Render Queue's deferred render target](../native/tools/capture-mrq.md), which constructs an offline view family. These images establish native shader execution and fixed-pose image output. They do not establish sustained frame rate, main-game temporal behavior, motion recovery or disocclusion. The ordinary viewport readback failure above remains unresolved; its failed output is not used in the comparison.

A first successful MRQ image revealed a default vignette. After explicitly disabling it, all 230,400 raw pixels are within one byte of the rounded source palette; the maximum difference from an unrounded standard sRGB value is 1.05028 bytes. The corrected images share the same display settings. Raw, corrected and failed evidence remains preserved rather than overwritten. PNG quantization, different reconstruction filters and selected-pixel reference convergence must be retained when interpreting the independent quality scores.

The [independent native quality report](../native/evidence/quality-20260905T204627830Z/README.md) checks 54 predetermined off-axis pixels with two 65,536-sample original-ray references per pixel. Stable point parity matches raw at 47/47 positions; the seven boundary positions remain in quality statistics. Linear RGB RMS against the Gaussian target is 0.271281 / 0.113124 / 0.00215019 for raw / TSR / analytic. All analytic channels fit the palette-derived PNG allowance, which does not establish zero shader error. Maximum reference-sequence disagreement is 0.00156487, a convergence diagnostic rather than a bound. TSR uses a different reconstruction filter, so this is a target-specific check, not an overall AA ranking. The runner reproduces exactly from relocated checked-in evidence without generated Unreal assets.

At that milestone, the remaining work was a moving main-game view with camera-derived analytic inputs and a working ordinary-game capture route. The following batch supplies both; synchronized presentation, uninterrupted motion and whole-frame timing remain open.

## Camera-following Unreal output

The [native gallery](../native/comparison.html) now uses the ordinary game viewport, with two matched triplets: a fixed camera and a camera that played continuously to t=2 seconds. The new analytic material reconstructs its perspective homography from world position, reciprocal linear view depth and raster derivatives. It uses the same frozen `1612267` kernel, source periods and Gaussian variance. The fixed materials remain controls. Separate autoplay maps make the moving scene viewable in a normal game window; these independently launched windows are not yet a synchronized native side-by-side.

The [geometry gate](../native/evidence/dynamic-homography-20260905T205748.185542Z/README.md) tests 758 quads across six poses, aspect ratios, jitter and float32 derivative models. Ordinary-family discrepancy stays below 0.000161 checker periods. Near-pole cancellation, large world coordinates, orthographic views and geometric silhouettes remain outside that result. All eight [dynamic material compiler jobs](../native/evidence/compile-material-20260905T205842.364084Z/report.json) pass; actual ordinary-game Metal captures additionally verify execution. The motion adapter marks declined/exhausted work magenta and invalid inputs red, with separate regime materials for further diagnostics.

The [sequence gate](../native/evidence/motion-prepare-20260905T210255.471500Z/sequence-report.json) verifies exact authored keys and matching raw/analytic evaluated camera transforms over both eight-second trajectories. The [live-map gate](../native/evidence/live-prepare-20260905T210929.504003Z/live-report.json) reloads each saved map and verifies autoplay and exclusive binding to its own camera. Runtime history reset at the deliberate eight-to-zero replay cut is not established by asset validation.

Plain `Shot` from a real game window supplies the six clean images, without MRQ or an offline view family. The [capture workflow](../native/tools/viewport_diagnosis.md) records preparation, source hashes, camera, sequence time, settings and observed PNG format. All six accepted runs have clean exits, no ensures and unchanged sources. The historical FrameGrabber failure remains unresolved. Offscreen composition export generated contaminated pixels and opened crash-report windows; it is disabled. Those failed artifacts remain negative controls for the image scorer.

| Gaussian-target linear RGB RMSE | At rest | After motion history at t=2 |
| --- | ---: | ---: |
| No AA | 0.267730 | 0.279330 |
| Unreal TSR | 0.113363 | 0.098215 |
| Analytic | 0.004138 | 0.004817 |

The independent [fixed](../native/evidence/game-quality-20260905T212821544Z/README.md) and [after-motion](../native/evidence/game-quality-20260905T212756753Z/README.md) reports retain all 54 predetermined quality pixels and use two 65,536-sample original-ray references per pixel. Dense raw registration passes 885/885 and 879/879 positions. Source, display transfer, preparation and observed cameras match across each triplet. The after-motion capture plays continuously from sequence frame 55 to 120, then pauses for **one additional stationary readback frame**. These scores do not measure uninterrupted temporal artifacts or frame rate.

The screenshots use a documented power gamma of 2.2. Three palette anchors validate those colors, not arbitrary sub-byte transfer accuracy. Nonzero analytic residuals remain beyond the unchanged display-code allowance: maximum 0.00351081 at rest and 0.00426881 after motion history. Their channel dependence demonstrates an output precision contribution, without explaining the entire error. The [residual notes](../native/evidence/game-quality-20260905T212821544Z/integration-notes.md) retain every failure and reference-convergence limit. TSR reconstructs a different filter; these target-specific scores do not establish a general winner.

Next is a synchronized native presentation with uninterrupted camera motion, cut/disocclusion tests, whole-image unsupported-regime counts, and one-method-at-a-time steady-state 1080p timing. The [live launcher](../native/tools/live.md) can collect raw boot CSV data, but startup, unsynchronized poses and desktop contention must be resolved before quoting a whole-frame gaming budget. No new speed claim follows from the successful screenshots.

## Candidate isolation and an uninterrupted game frame

The test servers now accept an [immutable candidate selection](../tests/compare/candidate-source.md): exact Git module bytes are archived and served only to that test run. The production shared module is not edited. Checker/circle gates select `OURS_KERNEL_CORE` where available, excluding unrelated ripple helpers. HLSL is compiled from the same module's own exported source.

The first `b05d227` / `3b81661` candidate failed actual DXC because its instrumentation wrote an undeclared `WORK` counter. That [failure](../native/evidence/candidate-20260905T213941.075Z/README.md) is preserved. The author's `6eddded` fix passes all eight real compiler jobs, 122 direct float fixtures and the bounded watchdog. The extra 22 fixtures independently integrate offset corners around correlations ±0.75 and anisotropic repeated-disc coverage. All 120 original scene probes, references and control outputs remain; maximum candidate/stable float difference is 1.49e-7, below the agreed 1e-4 gate. [Candidate evidence](../native/evidence/candidate-20260905T215650Z/README.md) retains exact source snapshots, outputs and numerical controls.

The performance runner now includes the actual homography arm. A stable/candidate/stable sequence at 640×360 and 1920×1080 produced 360 valid timestamp samples. Candidate medians were lower in all four cases, but the unchanged raw arm varied substantially: its 640×360, t=0 completed-wall medians were 9.8 / 1.1 / 1.7 ms. This prevents a reliable intrinsic speed factor. The [timing comparison](../native/evidence/candidate-20260905T215650Z/performance-comparison.json) preserves all three runs, GPU spans, completed queue times and limits. Five warm frames and 15 measured frames do not establish a sustained game budget. Production remains on `1612267`.

Separately, the [new native triplet](../native/evidence/game-quality-20260905T221241048Z/README.md) keeps its camera playing throughout screenshot readback. The request at sequence frame 120 saves frame 121, at 2.016666774 seconds. Independent raw registration passes 876/876 points and rejects neighboring frames. The other arms match the observed camera and readback phase; their saved phase is inferred from that common capture contract rather than separately recovered from filtered pixels. No pause, final seek, skip, cut or ensure occurred, and all processes exited with unchanged sources.

All 54 quality probes remain. Gaussian-target RGB RMSE is 0.279080 / 0.0961393 / 0.00433559 for raw / TSR / analytic. The analytic maximum residual beyond the unchanged display-code allowance is 0.00229292. These are results for one genuinely moving game frame; output precision, filter differences and reference convergence still apply. The [gallery](../native/comparison.html) now uses this triplet for its In motion view.

The next product step is a [synchronized native viewport](../native/tools/synchronized-view-design.md) with persistent per-pane histories, followed by the same TSR applied to both raw and filtered shading. Material-node coverage, bounded work/fallbacks, automatic lowering and incremental native cost must be measured explicitly. A useful material prefilter complements engine reconstruction; the current isolated shading comparison is only one part of that product test.
