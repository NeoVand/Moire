...
The native host now has camera-following analytic materials, verified matched camera sequences, and ordinary-game autoplay maps. `native/comparison.html` presents clean ordinary-game raw/TSR/analytic captures at rest and after motion history. All six captures have stable source hashes and no ensures; both triplets pass independent source, camera, palette and dense registration gates. The moving captures include one stationary readback frame, and small analytic output residuals remain documented. These establish native image output, not uninterrupted temporal quality or game frame rate. The earlier MRQ gallery and failed capture records are preserved. Offscreen composition capture is disabled after its ensures opened crash-report windows; use the successful real-window plain `Shot` route. Next: synchronized native presentation, uninterrupted motion and cut-history checks, unsupported-region counts, and controlled 1080p whole-frame timing. Coordinate GPU ownership before launches or measurements.
