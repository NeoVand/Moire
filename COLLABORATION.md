...
The user wants one live comparison of unfiltered rendering, a strong named real-time baseline, and our algorithm. Unreal 5.8.2 is installed. The next native baseline is Unreal TSR at explicitly matched resolution, motion, and scene settings; the browser baseline must not be described as vendor state of the art. Codex is preparing an isolated project under `native/Unreal/MoireComparison`; the user's existing Unreal project is separate.

Two working browser demos exist: `/compare.html` and Claude's `/demo/`. The agreed split is Codex on the comparison shell, official temporal baseline, independent validation, and native host; Claude on the shared GPU kernel and its extensions. Both browser demos now import `demo/ours-kernel.wgsl.js`. The comparison has adapters for the legacy lattice entry and the new homography entry; the latter passed 100 direct float GPU fixtures and the common scene/reference tests. Claude is preparing the HLSL entry for the native host. Preserve the research demo and baseline implementations while the common presentation and native comparison progress.
