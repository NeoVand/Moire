...
\begin{teaserfigure}
  ...
  drawing and the right half is a smooth greyscale field showing the same broad
  ...
  \caption{\figlead{No rasters anywhere.} Eight superpositions, each cut at the
  dashed rule: to its left the pattern as drawn, to its right the same
  parameters under the envelope view of \S\ref{sec:envelope} --- the stack
  averaged over one period of its own phase, which removes the carrier exactly
  and leaves the fringe field it was carrying. The same bands cross the rule
  textured on one side and smooth on the other, because the right half is not a
  filter applied to the left: both are the same scalar fields --- each family
  answering ``how far to your nearest member, and which one is it'' --- asked
  once per pixel, the same picture at any zoom, every number a live slider.
  \textbf{(1)}~Rings about two centres: hyperbolae where the radii differ by a
  ...
  instrument's opening scene, its double rosette pinned on the rule.
