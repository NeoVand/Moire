...
Exactly one edit changes the program: a layer's field \emph{expression}
(\S\ref{sec:exprs}). Since the expression is compiled into the shader rather than
interpreted by it, a new expression is a new material --- that is the whole of the
trade, and \S\ref{sec:exprs} prices both sides of it. Everything else about a
field stays a uniform: gain, extent, which layer carries it, and turning it off.
So the rebuild happens only when a \emph{character} of the expression changes, and
we make it invisible in three steps. The material is rebuilt per slot, so eleven
layers are undisturbed. The rebuild is debounced by $220$\,ms, which is longer
than the gap between two keystrokes, so typing \texttt{sin(3 * x)} costs one
compile and not eleven. And the renderer holds the last presented frame across the
build instead of clearing, so the canvas shows a stale picture for a few frames
rather than a blank one. Meanwhile the field editor's live preview is drawn by the
CPU evaluator, which needs no compile at all, so the author is never actually
waiting on the GPU to see what they typed.
...
\begin{figure*}[t]
  ...
  The first shows a bold open weave of a dozen ring arcs from two nearby centres;
  the second shows the whole two-centre rosette, its rings crossing in a lens
  pattern; the third packs the same rosette into a dense dark pinwheel of spiral
  ...
  \caption{\figlead{One scene, sixteen-fold of zoom, and the price of the floor.}
  The tool's own opening view --- two walking circle families, the project
  \Moire{} eases into on first load --- at three times its default scale.
  \textbf{(a--c)}~zoom $4$,
...
World $Y$ is up, and the flip to device coordinates happens in exactly one
place~\cite{Nehab2008}. Dragging moves the selected layer, in screen space, which
requires inverting the layer's own rotation onto the drag delta or the layer walks
away from the cursor. Holding \textsc{alt} rotates the layer about the world
origin rather than its own centre, which is the operation that makes a walking
family unwind; degrees in the interface, radians in the uniform, and a separate
field from the per-member rotation. Space or the middle button pans, the wheel
zooms to the cursor, and \textsc{shift} on any slider divides its rate.

The first paint waits on \texttt{compileAsync}, because presenting a blank canvas
and then a picture reads as a bug. During the wait the stage shows a mark; when the
program is ready, the tool eases two default concentric layers into the loaded
preset, using the same morph machinery. The first thing an author sees is therefore
not a static result but the mechanism moving into place, which turns out to be a
better explanation of what the tool is than the panel of sliders next to it.

Everything else is deliberately thin: one chrome panel on the left, collapsible to
a pill, and a PNG export that reads the canvas back. There is no document format,
no layer effects, and no inspector. The pattern is the document.
