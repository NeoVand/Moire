...
exist: every bound in them was forced by something an author could reach with a
slider that we could not yet draw fast enough, or correctly enough. This section
...
\begin{equation}
  ...
  \label{eq:stroke}
\end{equation}
then $\alpha(d)=1-\mathrm{smoothstep}(h-a,\,h+a,\,d)$, with $a$ the antialias
band of Eq.~\ref{eq:alpha}.
...
\begin{tabular}{@{}lrr@{}}
  ...
  \bottomrule
\end{tabular}
\end{center}\noindent The gap is a factor of $\zoomNoiseRatio$. So the floor
does suppress the speckle, and by a lot. But the same experiment prices it: even sampled perfectly, the clamped stroke
...
family over the pixel, which for a walking family we do not have; that is the
open problem of \S\ref{sec:future}. The floor predates this characterization; the
...
which distances cannot matter. Anything beyond $h+a$ is fully
transparent. The solver of \S\ref{sec:inverse} is therefore not asked for
$\min_n d_n$; it is asked whether any $d_n$ falls below $h+a$, and is
...
(\S\ref{sec:time}). A looping parameter is written into that
...
Nothing in the renderer accumulates. A frame is the twelve slots' uniforms
and the camera, evaluated afresh. An animated parameter is a function
$v(t)$ rather than an increment $v\leftarrow v+\dot v\,\Delta t$, so
scrubbing is exact and a recording is a sequence of clock values, not an
observation of playback. The bookkeeping, shared timings, and why a
$2160$p file need not play at $2160$p live in the supplemental material
(\S S3).
