...
\begin{figure*}[t]
  ...
  displaced centers; and rectangular hyperbolae under diagonal lines. The bottom row
  ...
  $\het\le1/4$ and fading out over the last tenth of that range, so a curve ending
  is the criterion's edge and not the pixel grid's. The curves come from the two
  index fields; the mask comes from their gradients; neither reads the image.
  Ordered by the fraction of the frame the criterion admits: $\atlasCombs\%$,
  $\atlasCircleLines\%$, $\atlasHex\%$, $\atlasCircles\%$, $\atlasControl\%$.
  Column~(b) is masked to a wedge because outside it the circles and the lines
  cross at a wide angle. Column~(d) is masked near the two centers, where the
  superposition is a rosette and not a fringe field at all. Column~(e) draws
  nothing, and there is nothing there: the criterion refuses the whole frame, and
  the gray panel is flat. Curves are also suppressed wherever consecutive level
  ...
  drawn there would be speckle rather than a curve. The whole bottom row costs two
...
translated polygons as Proposition~\ref{prop:closed}; each is short and stated
in place.
