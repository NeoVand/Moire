...
\begin{figure*}[t]
  ...
  level sets of the per-pixel winning character over it in crimson, drawn only
  where the weighted merit $\mu_k\le1/4$ and fading out over the last tenth of
  that range, so a curve ending is the criterion's edge and not the pixel
  grid's. Admitted fractions: $\atlasCombs\%$, $\atlasCircleLines\%$,
  $\atlasHex\%$, $\atlasCircles\%$, $\atlasControl\%$.
...
not a fringe field at all; and (e) in sparse rational pockets, where a
higher-order character of the hyperbola against the line family briefly
clears the threshold. The bright corridor along the asymptotes belongs to
the hyperbola family alone. Curves are also suppressed wherever consecutive level
  sets fall closer together than they can be drawn apart: at the center of (c)
  the winning character is singular and its level sets crowd without bound, so anything
