...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}
    ...
    \begin{scope}[shift={(img.south west)},
      ...
      \node[lab, text=ink, align=left, anchor=west] (gl) at (0.012,0.8495)
        {the graph of $D=\idx_1-\idx_2$};
      \draw[lead, ink!65] (gl.south) to[out=-100,in=45] (0.0625,0.7083);
      \node[lab, text=accent!88!ink, anchor=west] (cl) at (0.418,0.8761)
        {contours at $D\in\Z$};
      \draw[lead, accent!75] (cl.west) to[out=205,in=50] (0.352,0.7827);
      \node[lab, text=ink!80, align=center, anchor=south] (wl) at (0.285,0.9486)
        {steep:\\contours crowd};
      \draw[lead, ink!60] (wl.south) to[out=-85,in=95] (0.276,0.7490);
      \node[lab, text=ink!70, anchor=west] (rl) at (0.055,0.4507)
        {two ring families};
      \draw[lead, ink!55] (rl.east) to[out=10,in=205] (0.272,0.4923);
      \node[lab, text=cool!90!ink, align=right, anchor=east] (sl) at (0.995,0.8579)
        {the sum $\idx_1+\idx_2$:\\steep everywhere};
      \draw[lead, cool!75] (sl.west) to[out=-155,in=75] (0.8543,0.8098);
      \node[lab, text=cool!90!ink, align=left, anchor=west] (pl) at (0.552,0.9020)
        {flat only between\\the centres};
      \draw[lead, cool!75] (pl.south) to[out=-62,in=118] (0.752,0.6754);
    \end{scope}
  \end{tikzpicture}
  \Description{Four panels in two rows. Top row: two orthographic 3D
  renders side by side. In each, a floor patch carries two superposed
  families of fine concentric rings whose overlap forms a moire, and a
  translucent surface floats above it, joined to the floor by dashed
  vertical corner posts. Top left: the surface is the graph of the index
  difference, a twisted sheet carrying broad crimson bands that widen
  where the sheet lies flat and crowd into a wall between the ring
  centres; labels read the graph of D, contours at integer D, steep:
  contours crowd, and two ring families. Top right: the same floor under
  the graph of the index sum, a bowl covered in tightly crowded blue
  rings with one bold flat bar between the centres; labels read the sum:
  steep everywhere, and flat only between the centres. Bottom row: the
  central band of the same ring pattern seen flat from above. Bottom
  left: the difference's crimson contours drawn on it, riding exactly
  along the broad white fringe channels of the moire. Bottom right: the
  sum's blue contours drawn on the same pattern, everywhere at the scale
  of the rings themselves, with one solid blue bar between the centres.}
  ...
  \textbf{Bottom:} the central band of the same pattern, flat, with the same
  levels drawn on it. The difference's contours (left) fall exactly on the
  broad fringes the eye picks out; the sum's (right) run at the carrier's
  own scale and disappear into it, save the inter-focal bar.
