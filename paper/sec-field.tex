...
rather than claiming zero. Four further remarks.
...
\paragraph{The difference is a difference up to labeling.}
Nothing fixes the sign of an index field: relabel a family's members $n\to-n$
and $\idx$ flips while every curve, stroke and pixel stays the same. $D$ is
therefore defined per \emph{pair of labelings}, and the theorem holds in
whichever labeling makes its own hypothesis true --- the one where the carriers
nearly agree, so that $D$ is slow. In the opposite labeling the same slow
combination reads $\idx_1+\idx_2$: the classical \emph{sum}
moir\'e~\cite{Oster1964}. Where the two index gradients anti-align it is the sum
that beats slowly, and a difference-only reading misses fringes that stand in
plain sight. This is not hypothetical. Our first envelope view
(\S\ref{sec:envelope}) held $\idx_1-\idx_2$ fixed while it averaged, and every
sum-labeled fringe system in the frame vanished from it.
\S\ref{sec:visibility} folds both labelings into one criterion, the envelope
sweeps whichever one the criterion picks, and for lattice layers, whose index is
a vector, the quotient grows from a sign to a small integer combination of
generators (supplemental \S S3).
...
level-set families, a rotation for the radial pencil, a translation for a
lattice. A lattice's index is a \emph{pair}, so which combination of its two
generators rides the diagonal, and which way, is the convention choice of
\S\ref{sec:visibility} over again --- and it must be made per point, or fringes
