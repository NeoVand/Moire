...
\begin{figure*}[t]
  ...
  \end{subfigure}\hfill
  \begin{subfigure}{0.565\textwidth}
  ...
  survive every budget. Right: two fans of straight lines through two centers,
  overlaid with small crimson arcs and tick clusters strung along the
  horizontal axis through both centers, sparse far out and crowding toward
  each fan.}
  ...
  through the rationals along the axis, and the level sets of the per-pixel
  \emph{winning} character (crimson, drawn only where $\mu_k\le1/4$, computed
  by the reduction of this section) resolve into discrete commensurate
  pockets, one per admitted rational, dense toward the pencils exactly as the
  rationals are dense. The render never saw the curves.}
  \label{fig:selection}
\end{figure*}
