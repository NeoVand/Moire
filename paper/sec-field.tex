...
\begin{figure*}[t]
  \centering
  \input{fig-charhills}
  \Description{Two hand-drawn three-dimensional panels. In each, a flat floor
  patch carries two families of fine concentric circles around nearby centres,
  and a curved surface floats above it. Left panel: the surface is a ramp
  rising from left to right with a steep central cliff shaded by stippled
  dots; bold contour lines cross its flatter flanks at integer heights, and
  dashed vertical lines drop from three of them to bold hyperbola-shaped
  fringe curves on the floor, which fan out from the region between the circle
  centres like spokes. A vertical axis at the left is labelled D equals phi
  one minus phi two; labels read graph of the difference, contours at integer
  D are the fringes, steep: no fringe, and two ring families. Right panel:
  over the same floor the surface is a bowl covered in finely crowded faint
  contour rings, labelled the sum phi one plus phi two: steep everywhere here;
  the floor below shows only the two carrier families, labelled no fringe
  survives.}
  \caption{\figlead{Characters as hills over the pattern.}
  Two ring families of equal pitch, centres a few pitches apart (floor).
  Every integer combination $k_1\idx_1+k_2\idx_2$ is a height function over
  the same superposition, and the moir\'e question is which hill is flat.
  \textbf{Left:} the graph of the difference $D=\idx_1-\idx_2$. Its integer
  levels are contours on this hill, and they project down onto the confocal
  hyperbolae the eye picks out as fringes (dashed drops). Where the hill lies
  flat the contours spread and the fringe is broad; on the wall between the
  two centres (stippled) the same contours crowd at the carrier's own scale
  and no fringe survives. The heterodyne ratio \eqref{eq:ratio} is exactly
  this steepness measured against the mean slope of the carriers: curves are
  drawn bold where their spacing clears the carrier comfortably and thin
  where it approaches it. \textbf{Right:} the sum $\idx_1+\idx_2$ over the
  same floor is a bowl steep nearly everywhere at this separation --- its
  contours crowd into a texture at the carrier scale, and its fringe system
  never forms. The criterion of \S\ref{sec:visibility} is therefore a
  \emph{minimum over characters}, taken pointwise: the eye reads, at each
  point, whichever hill lies flattest.}
  \label{fig:hills}
\end{figure*}
