...
of~\cite{Amidror2009Theory})---and the metrology literature develops
...
construction itself does not supply the coverage \emph{between} the fringes---metrology reads intensity as fractional fringe order, following
...
fringe is an unmodelled side effect of drawing two dense patterns---nothing
...
rather than a distance composite, and Section~\ref{sec:zoom} measures what
...
filter~\cite{MansonSchaefer2013}---and for a single family at a legible zoom it
...
precisely the spatial modulation of how the two families' strokes fall relative to
one another---so \textsc{over} replaces the varying overlap with a constant one
and the fringe field it carries is flattened, while the mean ink is barely
