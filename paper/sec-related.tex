...
has to. The spectral theory also supplies profiles, not only loci: the
T-convolution theorem gives the moir\'e's intensity profile as a normalized
cross-correlation of the layer profiles, in the image
domain~\cite{Amidror1994TConv}, trapezoidal for binary
gratings~\cite{Patorski1976Profile}, and extended to curved layers as the
profile composed with the index difference~\cite{Amidror1998Fourier}. Our debt
to this theory is correspondingly large; \S\ref{sec:fringe} states what the
field form adds: exactness with stated local hypotheses, per-pixel stroke
widths, index fields with no closed form, and a drawable validity criterion.
...
it as the geometric approach, complementary to the spectral one, with curved
gratings read as indexed line families (Vol.~I, \S11.2
of~\cite{Amidror2009Theory}); the metrology literature develops
it~\cite{Bryngdahl1974Moire}, at book length in Patorski and
Kujawinska~\cite{Patorski1993Handbook}. This is the direct ancestor of our
Section~\ref{sec:fringe}, and our index field is its label made continuous and
evaluated per pixel. We claim no credit for the locus. The indicial
construction itself does not supply the coverage \emph{between} the fringes
(metrology reads intensity as fractional fringe order, following
Sciammarella~\cite{Sciammarella1965Basic}, and the spectral theory supplies
profiles as above), nor a statement of when the construction is meaningful.
Theorem~\ref{thm:fringe} supplies the first as a closed-form profile of the
actual compositing operator and \S\ref{sec:visibility} the second as a field
that can be drawn.

\paragraph{Diophantine approximation, singular optics, incommensurate structure.}
Once the visible beat is a character of $\cnt$, three literatures become
the same arithmetic. The continued-fraction identification of
\S\ref{sec:selection} is Lagrange's theorem on best approximations of the
second kind~\cite{Khinchin1964Continued}; the golden ratio is the fringe
desert because it is the worst-approximable
number~\cite{Hurwitz1891Annaherung}; the per-pixel scan is
Lagrange--Gauss reduction of a rank-two
lattice~\cite{NguyenStehle2009Gauss}. Circle-valued fields mint the
dislocations of singular optics~\cite{Nye1974Dislocations,
NyeBerry1974Dislocations, SoskinVasnetsov2001Singular}, authored here
rather than measured, and the fork grating is how holography prints
them~\cite{Bazhenov1990Screw}. Twisted bilayer graphene is the physical
instance of a rank-two counting map against a small
rotation~\cite{Bistritzer2011Moire, Cao2018Magic}: the same object as a
hex-lattice twist in the tool, read as a superlattice rather than as a
drawing. We claim the identification, not the theorems.
...
color separations do not beat~\cite{Ostromoukhov1994Rotated, Ulichney1987};
multi-color dithering extends the control to non-standard
...
\paragraph{Physical realization and sensing.}
...
archival venues a stratum of interactive moir\'e toys, web generators, and
...
represents it, predicts it, or stabilizes it under zoom. Procedural moir\'e is
likewise a staple of shader practice, going back to Glassner's tutorial
treatment~\cite{Glassner1997Inside} and Sen's product-delay
...
combines them, typically as \texttt{abs(p1 - p2)}, a \emph{value} composite
rather than a distance composite; Section~\ref{sec:zoom} measures what
that costs: with no stroke width to stabilize, the pattern is a different
pattern at every zoom level, and the displayed moir\'e is partly the
...
singularities~\cite{Knoppel2015Stripe} and as phase fields feeding a periodic
profile~\cite{Tricard2019Phasor}; neither carries a distance field, a fringe
law, or a moir\'e.
...
The machinery we borrow from is the distance-field tradition: Taubin's
first-order distance for implicit curves~\cite{Taubin1994Distance}, guaranteed
...
mask~\cite{Green2007Improved, Quilez, Quilez2019Distance}. Domain repetition
with per-copy transforms, and the neighbor-checking it forces, is documented
practice~\cite{QuilezRepetition}; \S\ref{sec:inverse} replaces its fixed
neighbor budget with a certificate. Our index window
...
we replace amounts to. The closest methodological neighbors are the certified-query and
range-analysis renderers, from Snyder's interval-based constrained minimization
on procedural geometry~\cite{Snyder1992Interval} to the implicit renderers
that bound a function over a spatial cell and subdivide until the bound
decides~\cite{Knoll2009, Keeter2020, Sharp2022}. Section~\ref{sec:discussion}
...
\paragraph{Explicit geometry.}
The nearest thing to an off-the-shelf answer is to emit the members as paths and
hand them to a vector renderer. That tradition is mature: resolution-independent
curves on programmable hardware~\cite{LoopBlinn2005}, random-access rendering of
general vector graphics~\cite{NehabHoppe2008}, GPU-accelerated path
rendering~\cite{KilgardBolz2012}, analytic rasterization under a polynomial
filter~\cite{MansonSchaefer2013}. For a single family at a legible zoom it
is the better tool, which we say plainly because our own first implementation was
of exactly that kind. Two things end it, and \S\ref{sec:vector} measures both.
The count of members meeting the frame grows without bound as the view widens,
while a field costs the same per pixel at every zoom. And the compositing is
wrong in the regime that matters: a path renderer antialiases each path
separately and combines the results with \textsc{over}, which is derived from the
assumption that the two coverages are \emph{uncorrelated} sub-pixel
areas~\cite{PorterDuff1984}. A moir\'e is that assumption failing: it is
precisely the spatial modulation of how the two families' strokes fall relative
to one another, so \textsc{over} replaces the varying overlap with a constant
one and the fringe field it carries is flattened, while the mean ink is barely
touched. Analytic coverage of a whole scene at once~\cite{Catmull1978} does not
have this defect, but it is not what per-path pipelines do. The distance
composite of \S\ref{sec:field} is a union coverage, not a per-path one, which is
why it does not arise here.
...
frequency analysis of aliasing~\cite{Crow1977, Mitchell1990} through
procedural noise and its filtering, surveyed by Lagae et
al.~\cite{Lagae2010} and Bruneton and Neyret~\cite{Bruneton2012}, to
normal-distribution methods for glinty
