...
$\lVert q_n\rVert \ge |r - nL|$ by the triangle inequality on~\eqref{eq:qn}---both independent of $\theta$.
...
\begin{figure}[t]
  ...
  itself in each regime, framed in its curve's color: below the marginal drift
...
Theorem~\ref{thm:window}---$(r-\varphi+G)/(s-m)$ and $(r+\varphi+G)/(m-s)$---which are slightly tighter than the single display \eqref{eq:window}. The
