  ...
  \begin{tikzpicture}
    \begin{axis}[paperaxis, height=5.3cm,
      ...
      \node[anchor=north east, inner sep=1.5pt, fill=white, draw=cool, semithick]
        at (rel axis cs: 0.615, 0.97) {\includegraphics[width=1.62cm]{figures/inset-convex-cross.png}};
      \node[anchor=north east, inner sep=1.5pt, fill=white, draw=warm, semithick]
        at (rel axis cs: 0.985, 0.97) {\includegraphics[width=1.62cm]{figures/inset-convex-flat.png}};
    \end{axis}
  \end{tikzpicture}
...
The solver exists twice, TypeScript and WGSL, checked against each other on
the GPU (Section~\ref{sec:protocol}) and against brute force over shapes,
spacings, offsets, and the marginal drift. $B=1024$ evaluations and a
$2^{16}$ span cap terminate the $m=s$ locus when $\theta\neq0$; neither
constant affects correctness where Theorem~\ref{thm:window} applies
(Section~\ref{sec:results}). The shipped upper bound uses the two
per-branch forms from the proof of that theorem, slightly tighter than
\eqref{eq:window}. Near $m=s$ the closed form takes the flat branch of
Proposition~\ref{prop:closed}, because the crossing index is of order
$1/|s-m|$ and \texttt{f32} is catastrophic there. Side counts above $24$
(unreachable from the interface) fall through to the scan.
