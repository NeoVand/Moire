...
\subsection{The guard: what the renderer actually needs}
\label{sec:guard}

The solver does not need $d(p)$ everywhere. Coverage is computed as
$1-\mathrm{smoothstep}(t-a,\,t+a,\,d)$ for stroke half-width $t$ and
antialiasing width $a$, so any $d$ above $t+a$ produces identical output. We
therefore fix a \emph{guard}
\begin{equation}
  G \;=\; \max\bigl(t+a,\ \tfrac{3}{4}s\bigr)
  \label{eq:guard}
\end{equation}
and require only
\begin{equation}
...
respect to the image. This is the only approximation in the exactness argument,
and it is a change of nothing: the renderer's own quantisation.

Two smaller consequences of the same observation come free. The solver takes an \emph{accept} threshold $t-a$: once a residual falls
...
\begin{equation}
  ...
  \label{eq:drift}
\end{equation}
the support value of the metric in the direction \emph{opposite} the walk. This is
the paper's smallest observation and one of its most consequential. The obvious
bound is $m \le \lVert\delta\rVert$, and $\lVert\delta\rVert$ is what a careful
implementor writes down first. But~\eqref{eq:drift} is smaller, sometimes much
smaller: $m=\max(|\delta_x|,|\delta_y|)$ for a square, and as little as
$\lVert\delta\rVert/2$ for a triangle whose vertex points along the walk.

The gap is not cosmetic. Section~\ref{sec:window} shows the search interval is
finite iff $m \neq s$ and has width $\propto 1/|s-m|$. Substituting
...
\begin{figure*}[t]
  ...
  \begin{subfigure}[t]{0.485\textwidth}
    \vspace{0pt}
    \begin{tikzpicture}
      \begin{axis}[paperaxis, scale only axis, width=7.1cm, height=4.2cm, name=resax,
    ...
    \end{tikzpicture}
    \caption{One pixel, worked. The naive index guess is $\figNaive$; the true
    nearest ring is $\figTrue$, $\figGap$ rings away and outside any small
    neighbourhood. The two envelopes of Lemma~\ref{lem:envelope} cross the guard
    band (pink) at $\figLo$ and $\figHi$, proving that no ring outside those
    $\figSpan$ indices comes within $G$. The Lipschitz rule then visits
    $\figEvals$ of them (dots), taking long strides while $|h|$ is large and
    closing in only near the crossing.}
    \label{fig:residual}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.485\textwidth}
    \vspace{0pt}
    \begin{tikzpicture}
      \begin{axis}[paperaxis, scale only axis, width=7.1cm, height=4.2cm, name=fanax,
    ...
    \end{tikzpicture}
    \caption{Why no constant works. Triangles, $\theta=0.02$, $96$ directions per
    radius; the stray is the worst seen out to that radius. The nearest index
    leaves any fixed neighbourhood of the naive guess and the gap grows linearly,
  ...
  \caption{The two facts that make bounded inversion possible. Left: the residual
  at a single pixel is sandwiched between two lines in $n$, which is enough to
  certify a finite interval. Right: why no fixed interval could have been enough,
  measured.}
  \label{fig:bounds}
\end{figure*}
...
\begin{proof}
For the lower end, $|h_p(n)|\le G$ requires $h_p(n)\le G$, and by
\eqref{eq:envelope} $\kappa|r-nL| - ns - \varphi \le h_p(n)$. On the branch
$nL\le r$ this reads $\kappa r - \kappa nL - ns - \varphi \le G$, i.e.
$n \ge (\kappa r - \varphi - G)/(s+\kappa L)$; on the branch $nL>r$ the left side
of~\eqref{eq:envelope} is increasing in $n$ while the requirement is unchanged, so
no smaller $n$ is admitted. For the upper end, if $s>m$ then
$h_p(n) \le r + n(m-s) - \varphi$, and $h_p(n)\ge -G$ forces
...
exceeds the widest neighbourhood our previous solver could even express. The same
...
\begin{algorithm}[t]
\caption{Ring scan. Exact when $\mathrm{span}\le B$ (Theorem~\ref{thm:window}).}
\label{alg:scan}
\begin{algorithmic}[1]
...
this is the difference between neighbouring pixels agreeing on \emph{which rings
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}
    \begin{axis}[paperaxis, height=4.6cm,
      ...
      xmin=0, xmax=260, ymin=-640, ymax=620,
      legend style={at={(0.5,-0.42)}, anchor=north, draw=none, fill=none,
                    /tikz/every even column/.append style={column sep=0.5cm}},
      legend columns=2,
    ]
      \addplot[draw=none, fill=accent!22, forget plot] coordinates
        {(0,-10) (260,-10) (260,10) (0,10)} \closedcycle;
      \addplot[cool, very thick] table [x=n, y=h, col sep=comma] {data/convex-shrinking.csv};
      \addlegendentry{$m < s$: one crossing}
      \addplot[warm, very thick] table [x=n, y=h, col sep=comma] {data/convex-marginal.csv};
      \addlegendentry{$m = s$: flat forever}
      \node[anchor=north east, inner sep=1.5pt, fill=white, draw=cool, semithick]
        at (rel axis cs: 0.70, 0.95) {\includegraphics[width=1.35cm]{figures/inset-convex-cross.png}};
      \node[anchor=north east, inner sep=1.5pt, fill=white, draw=warm, semithick]
        at (rel axis cs: 0.97, 0.95) {\includegraphics[width=1.35cm]{figures/inset-convex-flat.png}};
    \end{axis}
  \end{tikzpicture}
  \Description{A plot of the residual against ring index for two settings, with a
  pink horizontal band marking zero. The blue curve starts near five hundred, falls
  in a few straight segments, crosses the band once near index fifty and continues
  down off the bottom of the frame. The orange curve falls alongside it, then levels
  off onto a horizontal line well below the band and stays there for the remaining
  two hundred indices.}
  \caption{With $\theta=0$ the residual of a translated polygon is convex and
  piecewise linear in $n$, one segment per facet (hexagon, $s=14$). When the drift
  is below the spacing (blue) it falls monotonically and crosses zero once, on
  some facet's own segment, so the answer is a linear solve. At the marginal drift
  $m=s$ (orange) the leading facet's slope cancels and the residual is
  \emph{constant} past the crossover: all large indices are exactly equidistant, so
  no enumeration of finite length can settle them. The insets are the family
  itself in each regime, framed in its curve's colour: below the marginal drift
  the walking hexagons stay a drawing; at it the far side piles into a plateau.
  One evaluation of the constant
  settles all of them at once.}
  \label{fig:convex}
\end{figure}

The scan of Algorithm~\ref{alg:scan} is exact and fast, but for $\theta=0$ we can
do better than fast: we can be exact in $O(N)$ regardless of the interval width,
including on the locus where the interval is infinite. Property (P4) is the reason,
...
\begin{proposition}
\label{prop:closed}
Let $\theta=0$ and let the shape be a polygon with $N$ facets.
\begin{enumerate}
  ...
  \item If $m = s$, the facet $k^\star$ attaining $m$ has $s + b_{k^\star} = 0$, so
  $h_p \equiv a_{k^\star} - \varphi$ on the terminal segment. The distance is
  $\min(G, |a_{k^\star}-\varphi|)$ there, attained at every index past the
  crossover.
\end{enumerate}
...
for $p$ outside the innermost ring and $h_p\to-\infty$, it has one zero, attained
...
$h_p(n) = a_{k^\star} + ns - ns - \varphi$ is constant.
\end{proof}

The square case ($N=4$, normals on the axes) is the closed form we shipped first
and is presumably folklore; the content of Proposition~\ref{prop:closed} is that
it generalises to every side count and, more importantly, that case (2) exists.
Figure~\ref{fig:convex} plots both. The marginal case is the one that repays the
effort: it is exactly the locus where Theorem~\ref{thm:window} gives up, and it is
...
metric is a no-op); the quadratic for translated circles; Proposition~\ref{prop:closed}
