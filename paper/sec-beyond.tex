...
thirteen families (for the concentric and walking families the carrier
...
thin wherever the field is steep---by the factor $|\nabla\ph_2|$---which is the same eikonal bug \S\ref{sec:catalog} found in the curve families,
...
differentiated by hand and transcribed into WGSL and into TypeScript---two
derivations and four transcriptions per field, all required to agree and none
checked by a compiler. Adding a seventh field meant editing the renderer, so
...
the hand-derived gradients---the thing most likely to be silently wrong---are gone. Singular points are handled by clamps shared between the two
...
worked, and it cost $\interpPerOp$\,ms per megapixel \emph{per instruction}---$\interpHi$ for the longest preset, which at a retina canvas is most of a frame
...
\begin{figure}[t]
  ...
  row shows closed loops circling four centers arranged in a square, with saddle
  ...
  \caption{\figlead{Two things in one difference.} \emph{Top:} \Moire{}'s
