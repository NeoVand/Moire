...
Section~\ref{sec:object} said the exact rank-$1$ patterns form an affine
space over scalar fields: two line families that differ by $a s f$ have
index difference $a f$, so their fringes \emph{are} the contours of $f$.
This section takes that as a design primitive. The fringe law
(Theorem~\ref{thm:fringe}) read backwards is a contouring method that never
computes a contour.
