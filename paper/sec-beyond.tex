...
them, and not a discretisation of them --- the same curves a contour tracer would
be trying to find.

What is worth noticing is everything that does \emph{not} appear. There is no
...
little to ship. \S\ref{sec:catalog} writes every family as $\{\ph=ns+\phi\}$ and
inks it through $\mathrm{periodicDist}(\ph-\phi,s)/|\nabla\ph|$; encoding $f$
subtracts $a s f$ inside that residual and adds $a s \nabla f$ to the gradient it
divides by. Two extra terms, one shared by all thirteen families.

The gradient term is not optional. Omit it and the modulated family's strokes
thin wherever the field is steep --- by exactly the factor $|\nabla\ph_2|$ ---
which is the same eikonal bug \S\ref{sec:catalog} found in the curve families,
...
contribution is a menu. It was a menu. Our first version shipped six fields as
branches in the shader, each with its gradient differentiated by hand and
transcribed into WGSL and into TypeScript --- two derivations and four
transcriptions per field, all of which have to agree, and none of which the
compiler checks. That is not a design an author can extend, and it is not one we
could extend either: adding a seventh field meant editing the renderer.

So the tool takes an expression. \texttt{x\^{}2 - y\^{}2} is the saddle,
\texttt{cos(tau * r)} is the bullseye, and $\fieldCount$ presets
(Appendix~\ref{app:atlas}, Table~\ref{tab:exprs}) are starting points rather than
...
\paragraph{The gradient comes for free, and exactly.}
...
the hand-derived gradients --- the thing most likely to be silently wrong ---
are gone. Singular points are handled by clamps shared between the two
evaluators, so a pole is the same finite number on both sides rather than a
\texttt{NaN} on one.

One rule in that traversal is worth stating because getting it wrong cost us a
day. In the chain rule $\partial(g\circ h)=g'(h)\,\partial h$, a factor that is
...
have zero slope everywhere. So both evaluators multiply through a helper that
tests for an exact zero first. This is the ordinary hazard of automatic
differentiation in floating point, but it is easy to reach here because an author
can write \texttt{log(-1)} into a text box.

\paragraph{Do not interpret it in the shader.}
...
\begin{figure*}[t]
  ...
  that curve away from a bright X-shaped region through the centre. The second is
  the same state under the envelope view: smooth grey bands forming a hyperbolic
  ...
  curve running along the centre of a bright band, and the pink asymptote pair
  ...
  \caption{\figlead{A contour plot that was never contoured.} The Studio's
...
centred on the mathematical curve. That is a question about a rasterised image
...
Figure~\ref{fig:contourfields} --- the expressions the Studio offers as starting
points, not a set chosen for the experiment --- the mean displacement of the fringe peak from
the true contour is $\contourBestErr$--$\contourWorstErr$\,px, with a worst
$95$th percentile of $\contourWorstP$\,px. That is the strongest statement this
experiment can support: the fringe is on the curve to within our ability to say
where the fringe is.

\begin{figure*}[t]
  \centering
  \Description{Two rows of six square panels. The top row is black-on-white line
  patterns: a hyperbolic cross, two tight spiral nests side by side, three merging
  blobs, four ring nests in a square, concentric circles, and an irregular mottled
  texture. The bottom row shows the same six fields as smooth blue-to-yellow heat
  maps: a saddle, a dipole with one dark and one bright pole, three bumps, four
  vortices, a bullseye, and low-frequency noise. Each top panel's bands follow the
  contours of the map below it.}
  ...
  \caption{\figlead{Every preset the tool ships, encoded.} \emph{Top:} the rendered
  superposition. \emph{Bottom:} the field each one encodes.
  ...
  Table~\ref{tab:exprs}. The dipole is instructive: towards each pole the
  equipotentials crowd, and past the limit \eqref{eq:contournyquist} no carrier can
  resolve them --- so the render collapses into a dense scribble exactly where the
  field is steepest, which is an honest report. A contour tracer would have emitted
  confident polylines there.}
  \label{fig:contourfields}
\end{figure*}

\subsection{The limit, and what it looks like}
...
Two things about \eqref{eq:contournyquist} we like. First, it is a field, so it
can be drawn, exactly as the heterodyne ratio of \S\ref{sec:visibility} can:
...
steeper there than the chosen interval can show. This compares favourably with
the usual behaviour of a contour tracer, which will happily return a dense
thicket of polylines with no indication that the interval was the wrong choice.

\subsection{What else lives in a difference}

Equation~\eqref{eq:encode} is one line, and $f$ in it is arbitrary. Several things
follow that we have implemented and shipped but not studied, and we record them
here because the field formulation makes them cheap enough that not trying them
would be strange.
...
to trace before stopping. The Studio's \textsc{swirl} field is the stream function
of four point vortices, and Figure~\ref{fig:instrument} (top) is that field
encoded: closed streamlines around each vortex, and the separatrices between them,
drawn by two line families.
...
shadow moir\'e and moir\'e deflectometry measure with physical
gratings~\cite{Post1994High, Post2008Moire}, and it falls out of
...
against its own shadow on a bumpy surface (Figure~\ref{fig:instrument}, bottom).
...
whose ink is legible at any zoom. We have found this genuinely useful while
debugging the renderer in this paper, which is a small and self-referential
endorsement, but an endorsement.

\begin{figure}[t]
  \centering
  \Description{A two-by-two grid of square black-and-white panels. The left column
  is fine line work; the right column is the same content as smooth bands. The top
  row shows closed loops circling four centres arranged in a square, with saddle
  points between them, the pattern of a four-vortex flow. The bottom row shows broad
  bands bulging around blob-shaped regions, the pattern a warped grating makes
  against an unwarped one.}
  \includegraphics[width=\linewidth]{figures/instrument.png}
  \panelrow{0.49558}{0.00885}{%
    as rendered, envelope view}
  \caption{\figlead{Two things in one difference.} \emph{Top:} the Studio's
  \textsc{swirl} field --- the stream function of four point vortices --- encoded by
  \eqref{eq:encode}, so the fringes are the streamlines of the flow. No trajectory
  was integrated, no seed was placed, and the picture is a pair of \textsc{parallel}
  layers. \emph{Bottom:} \textsc{bumps} against the same carrier, which is a
  carrier against a warped copy of itself: the classical shadow-moir\'e arrangement,
  whose fringes are contours of the displacement along the carrier direction.
  \emph{Right column:} the same two states under the envelope view
  (\S\ref{sec:envelope}), which is how an author reads a field off the tool.}
