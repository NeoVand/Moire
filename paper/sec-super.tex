...
The joint counting map of a stack is $\cnt=(\idx_1,\dots,\idx_K)$. A visible beat is an integer character $k\in\Z^K$ of that torus: the scalar field $k\cdot\cnt$, slow against the carriers. This section is the arithmetic of those characters. The fringe law says the locally averaged ink is a function of $k\cdot\cnt$ alone. The heterodyne ratio $\het$ measures how slow. Weighted merit ranks the candidates. Selection is best approximation, answered by continued fractions in one dimension and by per-pixel lattice reduction in two. The envelope is the conditional expectation of $F$ over the subtorus orthogonal to the kept character.
...
Advancing an index by less than one does not change which members are
nearby, so the solver is asked once per pixel for a \emph{phase sample}
\begin{equation}
...
For the nine phase families this is exact. For a walking family or a radial
pencil it is a first-order model, and the choice of gap (local, not
nominal) is what gives the integral a unit period; the bookkeeping, the
widened guard, and each family's currency live in the supplemental
material (\S S3). The search runs once; the sweep is arithmetic. Median
envelope times fell from hundreds of milliseconds to within the $60$\,Hz
budget.

The gap is local, so a tap covers exactly one period of this family at this
point, whatever the pitch does across the frame. $\ell$ is where the
non-sliding parts live: a saturated guard, a pencil's hole, a hyperbola's
missing inner neighbor. Separating them from the residual is what keeps the
average from inventing ink there.
...
\begin{figure*}[t]
  ...
  \begin{subfigure}{0.36\textwidth}
    \begin{tikzpicture}
      \begin{axis}[paperaxis, height=3.7cm, width=\linewidth,
        xlabel={pitch ratio $\rho$}, ylabel={locked $p/q$},
        xmin=1, xmax=3.5, ymin=0.9, ymax=3.6,
        ytick={1,2,2.5,3},
        yticklabels={$1$,$2$,$\tfrac52$,$3$},
        legend columns=3,
        legend style={at={(0.5,1.04)}, anchor=south, draw=none, fill=none,
                      font=\scriptsize, /tikz/every even column/.append style={column sep=4pt}},
      ]
        ...
        \node[soft, font=\scriptsize, anchor=south] at (axis cs: 1.618, 3.42)
          {$\varphi$};
        \addplot[ink!35, const plot, line width=1.15pt]
          table [x=rho, y expr=\thisrow{p2}/\thisrow{q2}, col sep=comma]
          {data/convergent-fan.csv};
        \addlegendentry{$q\le2$}
        \addplot[cool, const plot, line width=0.9pt]
          table [x=rho, y expr=\thisrow{p4}/\thisrow{q4}, col sep=comma]
          {data/convergent-fan.csv};
        \addlegendentry{$q\le4$}
        \addplot[accent, const plot, line width=0.55pt]
          table [x=rho, y expr=\thisrow{p16}/\thisrow{q16}, col sep=comma]
          {data/convergent-fan.csv};
        \addlegendentry{$q\le16$}
        \node[font=\scriptsize, anchor=west, inner sep=1pt] at (axis cs: 2.08, 2.18)
          {$2{:}1$};
        \node[font=\scriptsize, accent!80!ink, anchor=west, inner sep=1pt]
          at (axis cs: 2.58, 2.68) {$5{:}2$};
        \node[font=\scriptsize, accent!80!ink, anchor=west, inner sep=1pt]
          at (axis cs: 3.08, 3.18) {$3{:}1$};
      \end{axis}
    \end{tikzpicture}
  ...
  \begin{subfigure}{0.62\textwidth}
    \includegraphics[height=3.7cm]{figures/selection-stations.png}
  ...
  ratio, hugging the diagonal, for three order budgets. The budget-two staircase
  ...
  survive every budget. Right: a wide strip of two dense fans of straight
  lines through two centers, cropped to the axis between them, overlaid with
  crimson arcs and tick clusters along that axis, sparse far out and crowding
  toward each fan.}
  ...
  Two radial pencils, cropped to the axis band: the local pitch ratio of
  their index gradients sweeps through the rationals along the axis, and the
  level sets of the per-pixel \emph{winning} character (crimson, drawn only
  where $\mu_k\le1/4$, computed by the reduction of this section) resolve
  into discrete commensurate pockets, one per admitted rational, dense toward
  the pencils exactly as the rationals are dense. The render never saw the
  curves.}
  \label{fig:selection}
\end{figure*}
