...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[scale=0.92]
    ...
    \foreach \n [evaluate=\n as \rr using \n*\sp] [evaluate=\n as \cx using \n*\dx*cos(\n*\th) - \n*\dy*sin(\n*\th)] [evaluate=\n as \cy using \n*\dx*sin(\n*\th) + \n*\dy*cos(\n*\th)] in {1,2,3,4,5} {
      \draw[soft, thin, rotate around={\n*\th:(\cx,\cy)}]
        (\cx,\cy) +(90:\rr/0.866) -- +(210:\rr/0.866) -- +(330:\rr/0.866) -- cycle;
      \fill[soft] (\cx,\cy) circle (0.7pt);
    }
    % highlight ring 4
    \pgfmathsetmacro{\rr}{4*\sp}
    ...
    \node[accent, font=\footnotesize] at (1.55,1.02) {$q_n$};
  ...
  little further than the last and each centred a little further from the origin, so
  their centres trace a short dotted spiral outwards. The fourth triangle and its
  centre are picked out in blue and labelled, and an arrow runs from that centre to a
  query point marked outside the triangles.}
  \caption{A walking family of triangles ($N=3$). Ring $n$ has inradius
  $ns+\varphi$, its centre $c_n = \Rot{n\theta}(n\delta)$ walks along the dotted
  spiral, and its own frame is turned by $n\theta$. Rendering asks for the
  distance from $p$ to the nearest ring; the residual of ring $n$ is measured in
  that ring's frame, on the pulled-back point
  $q_n = \Rot{-n\theta}\,p - n\delta$. Because each ring has a different frame,
  no single scalar field of $p$ has these rings as its level sets.}
  \label{fig:family}
\end{figure}
...
\begin{equation}
  ...
  \label{eq:family}
\end{equation}
for $n\in\N_0$, with spacing $s>0$, phase $\varphi$, per-index translation
...
concentric shapes; $\theta=0$ alone gives a family whose centres walk a straight
line; and letting $s\to\infty$ with the shape a half-plane recovers parallel
lines.
...
overestimates it by at most $1/\kappa$ for polygons. Renderers of this kind
consume $d$ through a smoothstep of width one pixel, so the distinction changes
apparent stroke weight near corners and nothing else. We keep the metric residual
because it is what admits the bounds below; Section~\ref{sec:limits} notes the
cost.

\subsection{Why inversion is the whole problem}

Equation~\eqref{eq:field} is a minimisation over an infinite discrete set, so
...
Section~\ref{sec:closed}, and it appears to be new.
...
pixel's radius,'' plus a fixed number of neighbours. The next section shows how
far that can be from the truth, and replaces it with an interval that cannot be
wrong.
