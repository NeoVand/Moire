\section{Folds and their inversion}
\label{sec:walking}

The fold rung of \S\ref{sec:object} is a family whose members each carry
their own rigid motion. There is no global counting map. An index field
still exists locally, wherever consecutive members bracket the point, and
the fringe law still applies to it. What is lost is the formula, so the
nearest index must be searched for. Search is the price of
non-integrability; the certificate of the next section is the interest
rate.
