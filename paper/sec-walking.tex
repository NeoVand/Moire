...
\begin{figure}[t]
  ...
  center are picked out in blue and labelled, and an arrow runs from that center to a
  query point marked outside the triangles.}
  \caption{A walking family of triangles ($N=3$). Ring $n$ has inradius
  $ns+\varphi$, its center $c_n = \Rot{n\theta}(n\delta)$ walks along the dotted
  ...
  index field survives only as the answer to a search (\S\ref{sec:inverse}).}
  \label{fig:family}
\end{figure}
...
\emph{under}estimates it for polygons, by a factor of at most $1/\kappa$---(P1) gives residual $\le$ distance always, and the gauge comparison gives
