...
Neo has set the goal, so these are decisions, not proposals. The result that changes the project's value is a side-by-side rendering: the same shaders, no anti-aliasing, supersampling at equal cost, the published state of the art, and ours, at real-time rates, noise-free, under a moving camera, with error meters against a converged reference. Something a graphics or game company can look at and buy, and a paper can be built on. Every choice below follows from that.

**Hardware, budget, metric.** The product target is the GPU, in the tool's WebGPU renderer: $1920\times1080$, at most $2$ ms of GPU time per material pass on a current consumer card, for the plain benchmark shaders first and the rippled and bumped variants next. Error is unclamped linear-light RMS and maximum against a $10^7$-sample reference at probe pixels, at equal GPU time to the baselines; temporal RMS at rest must be zero for the analytic path, since "no shimmer, no history" is the promise. The offline CPU reference (a $480\times320$ frame under $60$ s on sixteen threads, RMS $\le 10^{-4}$ at probe pixels) stays as the measuring stick that makes the numbers credible; it is not the product.

**Correctness.** The original source shader, always. A declared model is an implementation detail with a validity condition and a measured error, and the demo's meters show that error. Sampled residuals are acceptable offline, unbiased, for the reference and the paper's tables; in the real-time path only where their noise is invisible, which in practice means the analytic path everywhere it exists and a sampled fallback only for structures it cannot yet trace.

**The hard case and the frozen range.** The cases that decide the demo are the ones that look impressive side by side, so they come first: the checkerboard and circles planes at the horizon under a moving camera (the classic, where mipmaps blur and supersampling shimmers; our bias there must be gone, which is the exact-conditioning work); the rippled and bumped variants at grazing angles, where a mipmap cannot follow the parallax and a supersampler cannot afford it; and fire with bumps at $(300,12)$, $(400,60)$ and $(100,120)$ as the general-shader case, with bump amplitude and parallax height swept over $[0.5,2]\times$ their published values and time in $\{0, 0.37, 1.1\}$. Checkerboard $(240,60)$ stays as the case where I expect the controls to gain nothing. Frozen before tuning. The criterion for the sampled direction is unchanged: on the fire pixels, unbiased against the $10^7$ reference and at least three times better than plain sampling at equal total cost, or it is not worth pursuing there; and for the demo, whatever runs in real time and is visibly right wins.
