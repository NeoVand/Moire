...
Five, in decreasing order of severity. \emph{The character scan stops at order
two.} The criterion and the envelope handle the integer beats
$k_1\idx_1+k_2\idx_2$ with $\lvert k_i\rvert\le2$; higher-order moir\'es
exist classically~\cite{Amidror1994TConv, Amidror2009Theory} and metrology
multiplies fringes deliberately at order $N$, so a pitch ratio near $3{:}1$
still defeats the criterion exactly as $2{:}1$ defeated its first-order
predecessor. The scan generalizes mechanically; where to stop is a perceptual
question---contrast falls with order---that we have not measured.
...
continued-fraction expansion of $\theta/2\pi$. We consider it the most
promising follow-on. \emph{The metric.} We minimize the residual of the inradius metric, not
...
ratios $\het_k$ of Eq.~\ref{eq:ratiok} say where one will appear, which beat it
will be, and how it will run. And the fringe becomes \emph{addressable}:
