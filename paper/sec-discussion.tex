...
\paragraph{Why the discrete setting is favourable.}
It is worth naming why this problem admits a stronger guarantee than the
...
Yan2016}, tiled or stamped patterns~\cite{Lagae2010}, layered fibres --- have the
same structure available and are being solved with budgets instead.

\paragraph{On the value of a tight constant.}
The most transferable practical lesson is Section~\ref{sec:drift}. \loose{}
differs from \ours{} by one expression: $\lVert\delta\rVert$ instead of
$\rad(-\delta)$. Both are valid bounds. Both give a correct algorithm in the sense
that the interval contains the answer. But the loose one makes the interval too
wide to walk on a band of ordinary settings, and the fallback for a too-wide
interval is subsampling, so a bound that is merely \emph{loose} produces an image
that is visibly \emph{wrong} (Figure~\ref{fig:drift}: $\artDriftLooseDiff\%$ of
pixels, the largest fidelity error in the paper). When correctness is contingent
on a budget, the tightness of a bound is not a performance question.

\paragraph{What the representation bought, besides speed.}
...
debugging method. Three of the four curve families in the shipped shader were
thresholding a phase residual against a length, and the bug had survived because
its signature --- strokes thinning where a family steepens --- looks like a design
choice rather than an error. Writing $d=\mathrm{pd}(\ph-\varphi,s)/\lVert\nabla
\ph\rVert$ down as the definition of a layer made the missing divide a type error
rather than a matter of taste (\S\ref{sec:eikonal}, Appendix~\ref{app:atlas}). The
same thing happened three more times: the stroke floor turned out to be applied in
the wrong units for the same reason; the subsampling fallback turned out to have a
choice of anchor nobody had noticed was a choice (\S\ref{sec:results}); and writing
the envelope's contrast as an expansion about the stack's own mean coverage
(\S\ref{sec:envelope}) exposed that we were counting a lattice as one family when
it draws three, which put the pivot brighter than the picture and drove the frame
black at any real contrast. We would put it this way. A representation that names
the quantities forces the units to agree, and most of our bugs were units --- or,
in that last case, a count.

Naming them also tells you which quantities should never be written twice. Each of
our first six fields carried a gradient we had derived by hand and transcribed
into two languages, and the definition of a layer says that gradient is not an
attribute of the field but a consequence of it. Once that is admitted the
transcriptions are indefensible, and forward-mode differentiation
(\S\ref{sec:exprs}) removes them along with the class of bug they were the only
source of.
...
is. This bound bit us in exactly one place we wanted to go: displaying two encoded
...
coverage --- and we do not have one. It is the open problem in this paper we would
most like solved, because it is what stands between a moir\'e and a display with
more than one channel.

\paragraph{Limitations.}
Four, in order of how much they bother us. \emph{The marginal band.} On
...
continued-fraction expansion of $\theta/2\pi$. That is the paper we would write
next. \emph{The metric.} We minimise the residual of the inradius metric, not
...
Two of our thirteen --- radial pencils and lattices --- have no phase function in
the sense of \S\ref{sec:catalog}: the pencil's index is an angle, so its fringes
...
of marks and let the overlap do the rest. Doing it that way makes the fringe an
accident of sampling --- the thing you got, rather than the thing you asked for.
We have argued that a moir\'e layer is better described by two scalar fields, an
index and a distance, and that with those in hand the fringes stop being an
accident. They are the unit level sets of the difference of the index fields
(Theorem~\ref{thm:fringe}), a statement that needs no periodicity, no lattice,
and no Fourier transform, and that holds for every family we could write a phase
function for.
...
plate. The fringe becomes predictable before it is drawn, because a single local
ratio $\het=\lVert\nabla D\rVert / \lVert\nabla\bar\ph\rVert$ says where one will
appear and how it will run. And the fringe becomes \emph{addressable}: choosing the
index fields so that their difference is a field you care about turns the moir\'e
into a contour plot of that field, which we verified localises to
$\contourWorstErr$ px against ground truth on \contourFieldCount{} fields, written
as expressions and differentiated as they are evaluated so that a field the author
invents arrives with the gradient the renderer requires.
...
$\gpuSpeedLo$--$\gpuSpeedHi\times$ faster than the enumeration it replaces and in
exact agreement with brute force over all $\fidLegible$ legible settings we tested.

The work is embodied in \Moire{}, a browser tool of some $7{,}700$ lines in which
thirteen families, twelve simultaneous layers, authored fields, and every parameter
in this paper are live under the cursor at
$60$\,Hz. Building it is what produced the mathematics: the eikonal divide, the
pixel-relative stroke floor, and the drift interval were each found by dragging a
slider until something looked wrong. We would recommend the arrangement. A field
formulation is a good way to make a pattern precise, but an instrument you can move
is a good way to find out which parts of it are false.
