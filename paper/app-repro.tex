...
\begin{tabular}{@{}lll@{}}
  ...
  Ablation (Table~\ref{tab:ablation})    & \texttt{cost.mjs}       & \texttt{ablation.json} \\
  ...
  Instrument (Fig.~\ref{fig:instrument}) & \texttt{instrument.mjs} & --- \\
...
The comparisons in \S\ref{sec:results} are between three solvers, and none of them
is a reimplementation. \sweep{} and \ours{} are the two historical versions, kept
verbatim under \texttt{paper\pslash tools\pslash legacy\pslash}; the third is
\texttt{src\pslash gpu\pslash inverseCpu.ts}, the file the shader's CPU twin actually uses. All
three are loaded by rewriting their source at import time to wrap
...
solvers that search differently: in all three, one candidate index costs one
...
come from a CPU rasteriser that mirrors the shader's \texttt{composite.ts} term for
