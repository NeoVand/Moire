...
\emph{synthesis}: a smaller literature builds moir\'e on purpose---band
...
tradition one assumption is nearly universal: a layer is a raster---an image
...
them---and verifies it against $\fringeSamples$ measurements over
$\fringeScenes$ pairs of families to a mean coverage error of $\fringeMeanErr$.
...
the visible moir\'e is whichever low-order combination beats slowest
(\S\ref{sec:visibility}). The criterion, and the tool, scan the characters up
to order two.
...
artists actually reach for are families whose members are not concentric---each ring displaced a little further than the last, or turned a little further,
...
that provably contains every member passing within a stroke width of the pixel---an interval that, unexpectedly, does not involve the rotation $\theta$ at
...
interval genuinely explodes---the tangent-pencil locus, where unboundedly
...
\begin{itemize}
  ...
  difference, so fringes are its unit level sets---exact under stated local
  hypotheses, per-pixel, with no periodicity assumption, and with a computable
  validity criterion, the heterodyne ratio, scanned over the low-order
  characters and drawable as a map
  ...
  compiled to straight-line shader code rather than interpreted per pixel---which is what makes an arbitrary user-written field affordable, by
