...
into ink. The index field is a \emph{counting map}, advancing by one at each
member crossed, it is what the theory works with, and our central
...
The deeper organizing fact, and the paper's spine, is that a counting map can
exist in exactly three ways (\S\ref{sec:taxonomy}). For most families it is a
global formula; \S\ref{sec:catalog} lists the fourteen we ship, of which ten
are one line of algebra each. It can instead exist only up to an integer
gained around a loop: encode a \emph{circle-valued} field, the polar angle
being the prototype, and the count acquires a quantized winding, the fringes
\emph{end}, and the signed number of endings inside any loop is the enclosed
winding times the gain, exactly. That object is the fork grating of singular
optics, and Section~\ref{sec:defects} shows the fringe law failing at its core
radius and drawing a circle around its own failure. Or the count can exist
only as a search. What op
...
Two extra parameters, and the closed form is gone --- provably, not for want
of cleverness: the reach of formulas ends exactly at rational twists
(\S\ref{sec:walking}) --- each member lives in its own frame, no formula in
$p$ has these curves as its level sets, and the nearest index must be
searched for. The obvious search---guess the index from the
...
\begin{itemize}
  \item A field formulation of moir\'e in which a layer is a counting map plus
  a distance field, superposition is field compositing, and the pattern has no
  raster at any stage; with a three-rung taxonomy, exact, winding, fold, that
  organizes the catalog and the paper (Section~\ref{sec:field}).
  ...
  \item The fold law: where a walking family folds is decided by its support
  function in closed form before any solver runs --- rotation folds every
  shape but the circle, at radius $s/(\theta\sin(\pi/k))$ for the $k$-gon;
  translation folds exactly at supersonic drift, making the solver's drift
  constant a Mach number --- and the reach of closed-form inversion is
  exactly arithmetic: convex, hence solvable, for every translated gauge;
  splitting into $b$ closed forms for a rational twist $2\pi a/b$; provably
  transcendental beyond (Section~\ref{sec:walking}, verified against brute
  force in every case).

  \item The inversion of the transcendental remainder: a provably sufficient
  index interval from the same support function, a Lipschitz skip rule, and
  an exact closed form for translated polygons of any side count including
  the marginal (sonic) drift (Section~\ref{sec:inverse},
  Appendix~\ref{app:proofs}).

  \item \Moire{}, an interactive tool: fourteen families, twelve concurrent
  ...
  \item The winding rung made constructive: a circle-valued field mints
  topological defects, fringes that end, with the signed count of endings
  equal to the enclosed winding times the gain, exactly, and with the fringe
  law's own criterion drawing the core where it fails
  ...
  carrier pitch (Section~\ref{sec:beyond}).
