...
// ------------------------------------------------- paper 3: the theory outside moire
section('data/ear.json : an ear on aired pulse trains');
{
  const ear = load('ear.json');
  const failed = ear.gates.filter((g) => !g.ok);
  if (failed.length) {
    throw new Error(`ear.json has ${failed.length} failing gates; the prose claims none`);
  }
  const dh = ear.octave.depths.hard;
  const ds = ear.octave.depths.soft;
  macro('earHardNullSquare', th(dh['square-law']));
  macro('earHardNullCubic', th(dh.cubic));
  macro('earHardNullCascade', th(dh['square, pool, square']));
  macro('earSoftNullSquare', th(ds['square-law']));
  macro('earSoftNullCubic', r(ds.cubic, 1));
  macro('earLinearOverSquare', sci(Math.max(...ear.octave.hard.linear) / ear.octave.hard['square-law'][0]));
  const b = ear.beatsOfBeats;
  macro('earTernarySquare', sci(b.square / b.firstOrder));
  macro('earTernaryCubic', sci(b.cubic / b.firstOrder));
  macro('earTernaryCascadePct', r((100 * b.cascade) / b.firstOrder, 0));
  macro('earTernaryPrintedPct', r((100 * ear.printed.ternary) / ear.printed.firstOrder, 0));
  const strongest = Math.max(...ear.desert.goldenLines.map((g) => g.amp));
  macro('earDesertRatio', r(ear.desert.station / strongest, 1));
  console.log(`ear: ${ear.gates.length} gates pass; hard octave null ${dh['square-law'].toFixed(0)}x under a square-law ear`);
}

// The station exhibits of stations.mjs as a table: a few named ratios, their
// first convergents, and which of them the threshold makes a station.
{
  const st = load('stations.json');
  const name = (n) =>
    ({
      '16.4 : 8 = 41 : 20': '$16.4{:}8 = 41{:}20$',
      '15 : 5 twisted 2 degrees': '$15{:}5$, one turned $2^\\circ$',
      'sqrt 2': '$\\sqrt2$',
      e: '$e$',
      pi: '$\\pi$',
    })[n] ?? n;
  const rows = st.exhibits.map((e) => {
    const conv = e.rows
      .slice(0, 5)
      .map((q) => (q.station ? `\\textbf{${q.h}/${q.k}}` : `${q.h}/${q.k}`))
      .join(', ');
    return [name(e.name), r(e.x, 4), conv + (e.rows.length > 5 ? ', \\dots' : '')];
  });
  const tex = [
    '% GENERATED by paper/tools/numbers.mjs -- do not edit by hand.',
  ...
  writeFileSync(new URL('tab-stations.tex', PAPER), tex.join('\n'));
...
// ------------------------------------------------- paper 3: the predictions table, by domain
// The reader of paper 3 wants to know which claims reach outside drawings.
// Same data files as paper 2's table, a Domain column, and the ear rows.
{
  const d = load('dutynull.json');
  const e = load('exposure.json');
  const o = load('observer.json');
  const ear = load('ear.json');
  const st3 = load('stations.json');
  const conv = load('convergents.json');
  const fold = load('foldlaw.json');
  const def = load('defects.json');
  const x = load('exactsweep.json');
  ...
  const foldErrs = fold.t1
    ...
    .concat(fold.t2.map((c) => Math.abs(c.measured - c.nStar) / c.nStar));
  const rows = [
    ['A linear observer sees no beat in an additive superposition', 'any', '$' + sci(o.visibility.beat['additive, linear observer']) + '$', 'observer.mjs'],
    ['A squaring front end mints it at half the product of the harmonics', 'any', r(o.visibility.beat['additive, squaring observer'], 4) + ' vs ' + r(o.visibility.product / 2, 4), 'observer.mjs'],
    ['A window is the multiplier on the torus, with its curvature remainder', 'any', '$' + sci(Math.max(...o.multiplier.filter((m) => m.c === 0).map((m) => m.err))) + '$', 'observer.mjs'],
    ['Hard patterns are observer-proof', 'any', '$' + sci(Math.max(o.dutyNull[0].linear, o.dutyNull[0].squared)) + '$', 'observer.mjs'],
    ['Soft patterns reopen a null under squaring, linearly in the ramp', 'drawing', '$' + o.dutyNull.filter((q) => q.aa > 0).map((q) => sci(q.squared)).join('$, $') + '$', 'observer.mjs'],
    ['The octave duty null on paper: the $2{:}1$ station at coarse duty $1/2$', 'drawing', '$' + th(depth('2:1', 0.5)) + '\\times$ deep', 'dutynull.mjs'],
    ['The octave duty null in an ear: hard pulses, square-law, cubic, two-stage', 'sound', '$' + th(dh['square-law']) + '\\times$, $' + th(dh.cubic) + '\\times$, $' + th(dh['square, pool, square']) + '\\times$', 'ear.mjs'],
    ['A linear ear hears no beat in a sum of trains, at any duty', 'sound', '$' + sci(Math.max(...ear.octave.hard.linear) / ear.octave.hard['square-law'][0]) + '$ of the square-law ear', 'ear.mjs'],
    ['Softened pulses: a square-law ear keeps the null, a cubic ear reopens it', 'sound', '$' + th(ds['square-law']) + '\\times$ against $' + r(ds.cubic, 1) + '\\times$', 'ear.mjs'],
    ...
    ['Fringe endings count the enclosed winding; the core radius $2As/\\pi$', 'drawing', def.probes.filter((p) => p.expect !== null).length + ' probes exact; core within $10^{-4}$', 'defects.mjs'],
    ['The fold law: onset radii and the Mach condition', 'drawing', 'within ' + r(Math.max(...foldErrs) * 100, 1) + '\\%', 'foldlaw.mjs'],
    ['The pooled view is the slide average, exactly', 'instrument', '$' + sci(x.worst.exact3) + '$ over ' + x.scenes + ' scenes', 'exactsweep.mjs'],
  ];
  const tex = [
    '% GENERATED by paper/tools/numbers.mjs -- do not edit by hand.',
    ...
    '\\midrule',
    ...rows.map(([a, dom, c, sc]) => a + ' & ' + dom + ' & ' + c + ' & \\texttt{' + sc + '} \\\\'),
    '\\bottomrule',
    '\\end{tabular}',
    '',
  ];
  writeFileSync(new URL('tab-predictions3.tex', PAPER), tex.join('\n'));
  console.log('paper 3 predictions table: ' + rows.length + ' rows');
}
