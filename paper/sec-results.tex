...
\begin{figure*}[t]
  ...
  \begin{subfigure}[b]{0.43\textwidth}
    \begin{tikzpicture}
      \begin{axis}[paperaxis, width=\linewidth, height=4.9cm,
        xlabel={members sharing a pixel, $1/(s\,\text{zoom})$},
        ylabel={fringe contrast lost (\%)},
        xmode=log, xmin=0.5, xmax=12, ymin=-4, ymax=104,
        legend style={at={(0.5,-0.42)}, anchor=north, draw=none, fill=none,
                      /tikz/every even column/.append style={column sep=0.26cm}},
        legend columns=3, mark size=1.9pt,
      ]
        % The one-member-per-pixel band. This has to be a filled path, so `only
        % marks` cannot live on the axis -- it would apply here too and the region
        % would vanish. Each scatter carries it instead.
        \addplot[draw=none, fill=soft!20, forget plot] coordinates
          {(0.5,-4) (1,-4) (1,104) (0.5,104)} \closedcycle;
        \addplot[cool, only marks, mark=*] table [x=parallel-rotate_mpp, y=parallel-rotate_lost, col sep=comma]
          {data/vector-conflation.csv};
        \addlegendentry{two combs}
        \addplot[accent, only marks, mark=square*] table [x=circle-parallel_mpp, y=circle-parallel_lost, col sep=comma]
          {data/vector-conflation.csv};
        \addlegendentry{circles, lines}
        \addplot[warm, only marks, mark=triangle*, mark size=2.4pt] table [x=hexagon-hexagon_mpp, y=hexagon-hexagon_lost, col sep=comma]
          {data/vector-conflation.csv};
        \addlegendentry{hexagons}
        \addplot[ink, only marks, mark=diamond*, mark size=2.4pt] table [x=circle-circle_mpp, y=circle-circle_lost, col sep=comma]
          {data/vector-conflation.csv};
        \addlegendentry{two centres}
        \addplot[soft, only marks, mark=pentagon*] table [x=hyperbola-parallel_mpp, y=hyperbola-parallel_lost, col sep=comma]
          {data/vector-conflation.csv};
        \addlegendentry{hyperbolae}
      \end{axis}
    \end{tikzpicture}
  \end{subfigure}
  ...
  fringe contrast lost against members sharing a pixel, on a log axis, for five
  scenes. Below one member per pixel, in a shaded band, all points sit near zero;
  past two members per pixel most points rise above eighty percent.}
  ...
  \emph{Right:} that loss against how many members share a pixel, over
  $\vecSettings$ settings reached with the tool's own zoom and spacing controls.
  In the shaded band a pixel sees at most one member and \textsc{over} is
  accurate---this is the regime a vector back end is built for. The rise past it
  is not an antialiasing artifact, since both sides are given exact per-path
  coverage; it is \textsc{over} pricing the overlap of two families as
  $\alpha_1\alpha_2$ when the correlation between them \emph{is} the moir\'e.}
  \label{fig:vector}
\end{figure*}
