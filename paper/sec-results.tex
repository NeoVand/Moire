...
\begin{figure*}[t]
  \centering
  % The strip and the plot are set to one height, so the row reads as a single
  % graphic. The strip is 3 square panels and 2 gutters, 1220px wide, so at
  % 0.685\textwidth each panel is square and the block is 0.2246\textwidth tall;
  % the axis box is set under that to leave room for its own labels.
  \begin{subfigure}[t]{0.685\textwidth}
    \includegraphics[width=\linewidth]{figures/vector-wash.png}
    \panelrow{0.32895}{0.00658}{%
      (a) union coverage,
      (b) per-path \textsc{over},
      {(c) difference, $\vecFigAmp\times$}}
  \end{subfigure}\hfill
  \begin{subfigure}[t]{0.30\textwidth}
    \begin{tikzpicture}
      % The strip beside this is 0.685\textwidth of a 510pt text width at 1220:400,
      % so its panels stand 4.03cm tall. `height` here is the axis box plus its
      % labels, so it is set above that and only the x-label hangs below the
      % panels' own captions. Sizes are absolute because \linewidth inside a
      % subfigure is the subfigure's, and inheriting it made the plot a third the
      % height of the images it sits next to.
      \begin{axis}[paperaxis, width=5.3cm, height=5.2cm,
        ...
        % Where a pixel still sees at most one member: the regime a path renderer
        % is built for. A filled region, so `only marks` cannot sit on the axis --
        % it would apply here too and the band would silently vanish.
        ...
        \addplot[accent, only marks, mark=*, mark size=1.6pt,
