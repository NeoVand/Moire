...
**Materials with a range oracle: brackets [the envelope proved and audited; the counts mine, under audit].** The implicit class is one instance of something more general. Suppose that for every cell of a mesh we can certify a lower and an upper bound on the material, $\ell_C \le F \le u_C$ on the cell, as interval or affine arithmetic does for any shader program. Build the state from the lower brackets, exactly, cell by cell. Then for every pixel of the band the response error is one-sided and at most the pixel-density bound $1/(2\pi q_-)$ times the total gap $\int_W (u - \ell)$ over the window, plus the tail outside the window: the same two terms as before, with the gap in place of the uncertain area. Nothing else is charged. No coefficient of the state needs to be accurate in any norm, the refinement is driven by one number, the $L^1$ gap, and if the gap's own state is compiled alongside, each pixel can read off its own error bound. The count is where care is needed, and our collaborator caught me overclaiming it: constant brackets on a smoothly varying region force a cell size proportional to the tolerance and a cell count proportional to its inverse square, because the gap of a cell is its oscillation times its area. Affine brackets fix that, since their gap falls with the square of the cell size, and with the cells that meet the jump curves refined further the count is linear in the inverse tolerance, with the window area times the curvature bound and the squared jump length as the geometric factors (`orientation-note.md`, section 7j). What the theorem does not do is bound the oracle's overestimation for a given program. A hash evaluated afresh at every scale gives no useful bracket at all, which is why noise needs its own route; our collaborator has one from the carry state of Section 11, in the next paragraph, expanding the pixel's Gaussian cell by cell instead of the source, so that the error is a property of the kernel and not of the hashed values. Acquiring a shader adaptively under conservative bounds and pricing the leftover uncertainty under the pixel filter is older than this program: Heidrich, Slusallek and Seidel (1998), and Greene and Kass (1994).

**Noise with sharp thresholds: prototype moments [proved, our collaborator; audited twice, and checked here].** Recall from Section 11 that a lattice noise built from a hash with the carry structure has, in each unit cell, a local function fixed by a finite state (the cell's key and its two carry classes), so the number of distinct local functions is finite, at most $P D_x D_y$ for a period $P$. That state fixes the whole local function, not only its polynomial coefficients. So take any bounded pointwise response of the noise, a threshold included, and store for each state the moments of the complete response against the tensor Bernstein basis of degree $J$ on the unit square, normalized so that every entry lies in $[0, 1]$. The pixel's Gaussian is then approximated cell by cell by a polynomial of degree $J$ along each axis, its coefficients fold over the period, and the response is a contraction of the moment table against the folded coefficients, diagonalized by the Walsh transform as in Section 11. Three things make this the right shape. The polynomial degree belongs to the smooth Gaussian, not to the hashed material, so a threshold costs nothing in degree: $J$ grows like $\log(1/\varepsilon)$, and with the Gaussian's own Taylor polynomial and the constant-one Hermite bound of Szász a little slower still. Errors in the table are amplified by less than sixteen whatever the degree, because the folded coefficient norms stay below four per axis. And every footprint width of at least one cell is covered, the wide ones by the observation that a Gaussian much wider than the period is uniform on the torus up to an exponentially small error. What it does not do: the tables must be acquired (for rational polynomial thresholds, by the certified quadtree of the implicit class on the unit square, at a finite but large cost); a footprint narrower than a cell is out; a correlated footprint is not a product over the grid axes, and is handled by writing it as a positive mixture of a few axis-aligned Gaussians of equal width with centres along a line, one for each node of a trapezoid rule over a latent variable, whose number grows like the square root of one over one minus the correlation times the logarithm of the allowance, and which keeps every error bound of the product case because the weights are positive (our collaborator's, audited; the mixture exists whenever no direction of the footprint is narrower than a cell; my signed expansion by Mehler's formula is the fallback, with a worse rank and amplification); and editing the threshold changes the table, though not expensively: our collaborator stores, for each prototype, the cumulative moments at a ladder of quantile knots of its local field (knots spaced by an area $\eta$, found by a terminating rational bisection with an explicit polynomial modulus), and a change of threshold selects the bracketing knots per prototype, interpolates between them (so the response moves continuously with the slider, with no staircase) and rebuilds the Walsh tables once, after which every pixel's query is unchanged; the source error is one-sided and below $4\eta$ under the true footprint, because the disagreement area per cell is at most $\eta$ and the cell suprema of a Gaussian sum to less than four. That makes a threshold slider a table rebuild, not a re-acquisition (`THRESHOLD-CONTROL-STATE.md`, audited). It is a compile-time closure for a whole family of nonlinear procedural materials that the source-polynomial route could not reach (our collaborator's `NOISE-PROTOTYPE-MOMENTS.md`).

**Where the cost goes: a compact state for a whole hashed material [mine, under audit after three rounds of repairs; the coherent decomposition and the lookup are our collaborator's, audited].** The prototype route still scans every table entry for every pixel. For a fixed material that scan is not necessary, and the reason is structural. The Hermite state of Section 11c is an integral of the material against a product of one-dimensional Gaussian-Hermite kernels along the grid axes, and the material's tables form a periodic field on the cell grid, so every entry of the state, at every centre and for a given footprint width, is a periodic separable convolution of that field. One FFT pass per entry and width produces the state at every stored centre at once, the stored centres need only cover one period of the hash rather than the texture, and a pixel then does a lookup of its reference, a short recurrence and an inner product of a hundred or two terms, never touching a table. What sets the size of the state is not the material but the admission geometry: how far a pixel's centre may sit from its reference and how far its covariance may deviate, together with the bits wanted. The honest budget then exposed a trap, which our collaborator caught: if the geometric error of acquiring the tables is charged per entry and pushed through the signed Hermite weights, the tables would need an accuracy three to four orders of magnitude below the response accuracy, and acquiring them would dominate everything. The way out is to charge the geometry once. Compare the material to one coherent approximation of it (the same resolved-cell approximation of each prototype, wherever that prototype occurs) directly under the pixel's Gaussian, where the error is at most four times the disagreement area per cell, because Gaussian convolution with a wider footprint factors through the unit one and positive convolution cannot increase a supremum; then acquire the state of the approximation, whose tables are exact rational integrals over rectangles. Nothing geometric passes through the weights any more; only arithmetic does. Our collaborator's certified scale-space lookup, a mip-like table read at sixteen entries with a proof, is the natural comparator: it needs a few hundred times more storage than the state at eight bits, and its acquisition is one certified query per node, while the state's acquisition is the tables plus a negligible FFT; the state pays for that with a few hundred operations per pixel against sixteen reads. Both are compile-time constructions, and both are invalidated by any edit of the material's prototypes, which first re-acquires the tables and then rebuilds; neither is invalidated by the camera, the footprint, or a translation of the whole material. The numbers are printed evaluations of the certificates' formulas at a period of $256$ cells, not measurements (`orientation-note.md`, section 7m; `state-pyramid-budget.mjs`). Two refinements are now our collaborator's published and audited notes: the coherent source acquisition, and an orthogonal whole-kernel state in which the Gaussian-Hermite kernels of every degree share one envelope, so the table degree is of order the square root of the state degree and the table error of the whole state is bounded collectively by a shared error polynomial, with one assembled budget. And a third comparator is on the table: cache the material blurred at dyadic widths and serve any wider footprint by a positive lattice convolution with the residual Gaussian, an architecture that goes back to Feline (1999); our collaborator's certified contract for it stores under three million values at eight bits for the same material and reads at most a few thousand per pixel by a deliberately conservative count (a single cached level needs about eight hundred by my count), because the residual Gaussian does exactly what the Hermite expansion approximates. That candidate is not yet a contract, and the question of which of the three the product should carry is where the theory now stands.

## 12. Where we are, honestly

**Proved or audited.** The Gaussian character formula and the algebra of complex Gaussians (Section 7). The count theorem with its three branches and the level (Section 5; conditional on a certified cell integrator). The wrapped density lemma (Section 6). The projective density bound, its signed first correction with the finite hierarchy, and the positive quadratic model's bound (Section 8). The pathwise envelope, the fixed-weight bias and variance accounting, and the yardstick (Section 9). The band-mass certificates, single, wrapped, and graph-level (Section 10). The hash contraction and the carry-state theorem (Section 11). The one-dimensional Gaussian subdivision cost theorem (Section 11b, our collaborator's). The heat-content lemma, the tail-energy law with its constant, and the two-factor truncation certificate for filtered products (Section 11c). The orientation refinement of that certificate for polygonal materials, uniform, transverse, and by hyperbolic regions without any angle (Section 11c, orientation-note.md). The certified acquisition of implicit polynomial-threshold materials with its direct query-family budget (Section 11c, end; our collaborator's, counts checked here). The bracket envelope for materials with a per-cell range oracle (Section 11c, end; the affine-bracket count is mine and under audit). The prototype-moment closure for bounded nonlinear responses of carry-state noise, every footprint width of at least one cell (Section 11c, end; our collaborator's, audited twice, checked here). The threshold control of such a material by quantile knots with a one-sided source error below $4\eta$ (Section 11c, end; our collaborator's, audited).
