...
Convince yourself that a Gaussian footprint with $\sigma = 0.5$ pixels places $95$ percent of its mass within one pixel of the centre ($|Z| \le 2$ has probability $1 - e^{-2} \approx 0.86$; check the two-dimensional formula $P(|Z| \le R) = 1 - e^{-R^2/2}$ by integrating in polar coordinates). Also compute $\mathbb{E}[Z_1^2 Z_2^2] = 1$ from independence. These are the kind of small facts the constants in Section 8 are made of.
...
- For an affine $\Phi$ the count distribution is Gaussian and its pairing with any periodic $g$ is a closed form (Section 4).
...
Here $D_0$ is the depth at the pixel centre and $d$ its gradient per $\sigma$, so $k$ is the relative change of depth across one $\sigma$: the perspective rate. On our benchmark plane it is $|k| = 0.5/(y + 1)$ with $y$ the row counted from the horizon; it is $0.05$ at row $9$ and $0.012$ at row $40$. For every count of a material on the same plane, $k$ is the same vector: all counts share the denominator. That shared state is what makes the next results material-independent.
...
So if we can bound the total variation between the true count distribution and the affine one, we have bounded the mean error of every bounded material on that plane at once, thresholds, products, noise, anything, with a constant that does not care how fine the pattern is. Note what is being compared: two distributions of the whitened screen point, the Gaussian and the Gaussian pushed through the map $z \mapsto z/(1 + k\cdot z)$ that turns the affine count into the projective one.
...
This is the first-order change of the footprint's density under perspective: a signed reweighting by the polynomial $(k \cdot z)(3 - |z|^2)$. It is negative where the depth is larger and the point is within $\sqrt{3}$ of the centre, positive beyond, and the reverse on the near side: the footprint's mass shifts toward the far side and stretches, exactly what a perspective view does to a disc.
...
Put numbers on the benchmark camera, with the checkerboard's radiance range $W = 0.76$ and an eight-bit budget of $1/256$. The affine model is certified within budget only far from the horizon (row $71$ and below, since $0.76 \cdot 0.7381 \cdot 0.5/(y + 1) \le 1/256$ needs $y \ge 71$). The quadratic model is certified from row $15$ down. The corrected affine model is certified from row $9$ down. Rows $9$ to $15$ from the horizon are the heart of the transition band. Before these theorems, our kernel used the quadratic model there because it worked in tests; now it has a material-independent certificate, and a cheaper corrected affine model has one too. [measured] On the checkerboard the corrected model's actual error is $0.00015$ against its bound $0.00082$ at row $20$, and the correction removes $97$ percent of the affine model's error there (`projective-correction.mjs`). The price in retained modes is small: under a term-by-term tail the retained set is unchanged to within two percent, and under the conservative summed certificate it grows by a factor between $1.07$ and $1.5$ (`corrected-tail-count.mjs`).
...
This needs no independence, no stationarity and no correct reprojection. Its strength is also its limit: a structured pattern whose predictor is exact never ghosts or flickers, but the guarantee is only as good as the residual's envelope, and a residual that carries the aliasing (a threshold whose model is slightly off, a geometric edge) has envelope one. Section 10 is the certificate for that case.
...
The count-map view still applies: the counts are the lattice cell index and the position within the cell. Because the interpolation weights are the same function in every cell, the noise's Fourier transform is the interpolant's transform times a lattice sum of the hash values, and for a hash with period $P$ that lattice sum is a discrete Fourier transform of size $P$. [proved] The pixel's filtered value is then a contraction of the hash's spectrum against the Gaussian weights, computable in $O(P\log P)$ time per pixel with $O(P)$ storage. For products of noises the story is subtler: [proved] for hashes that are affine over the binary field, the composition state a compiler must track has at most $b^2$ bits for a $b$-bit hash, with explicit per-pixel error bounds. And there is a counterexample showing that a small state does not imply good noise: a Gray-code hash has few classes and zero neighbour covariance.
...
**Open, and it is the important one.** Cost. The kernel takes $7$ to $8$ milliseconds at $960 \times 640$ on an Apple GPU for the checkerboard, which extrapolates to about $25$ milliseconds at $1080$p against a product target of one millisecond; the mask is far worse. The theory now says where that cost is: not in precision, not in the representation, but in the constant factors of the transition band and in the joint integrals of masks. The certificates of Section 8 give the cheap models a licence to be used there; making them fast is engineering with a theorem behind it, and it has not been done. Generality is the other open front: textures, normal-mapped lighting and geometric edges stay with TAA, which the residual architecture makes acceptable but not free.
