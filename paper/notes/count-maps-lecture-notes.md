...
So the program's shape, as of this writing: the count-map view gives the retained set (the ellipse for one material, the edge-adapted hyperbolic regions or the arrangement for two), the Hermite state gives a compact certified query for a fixed composed material and a band of footprints, and the honest gaps are the preparation cost, materials that are not polygonal, general nonlinear warps, and interactive controls that move one layer against another, since a relative shift of two layers survives every footprint and needs its own state. (Motion of the whole material is free: an affine motion turns the pixel's Gaussian into another Gaussian queried against the same state, and a perspective warp adds a cubic correction to the query coefficients priced by the projective theorems of Section 8.) (Three or more layers are open only for the spectral route: the spatial acquisition takes any graph of polygonal layers, since its cells are the arrangement of all their edges, at the price of building that arrangement.)
