...
A draft for the collaborator's audit, 2026-09-06. The cell side and its certificate are theirs (SHARED-PHASE-AVERAGING.md and SHARED-PHASE-COST.md in the theory-program review folder); the spectral side and its certificates are from the bridge (#112, #116, #120). Theory only: nothing here is a GPU cost or a frame budget, and the arithmetic model is the collaborator's (exact reals, sorting, Gaussian CDF and density).
...
1. Assumption (b) for the family: coefficient bounds |H_hat(k, l)| from the Walsh expansion of F (total Walsh mass at most 2^{m/2} for 0 <= F <= 1) and the arc coefficients (at most 1 / (pi |l_j|) per index), and absolute convergence along every strip |k omega + l . delta| <= T for m >= 2.
...
Two carriers with nonparallel screen rates and slow shifts on each. The cell side becomes an arrangement in two phase dimensions, where two boundary directions (N, 1) and (1, N) already meet N^2 - 1 times per period, so winding must enter the event budget explicitly. The spectral side keeps the identity with (a) and (b), the windows become the material-intrinsic ladder of the pair (the integer points near the kernel of the surface rate matrix, camera-free, with the cutoff set by the camera), and (b) is the open question for thresholds of smooth fields, where the boundary's curvature controls the coefficient decay only generically. This is where the noise mask lives, and where the theory is not yet a theorem.
