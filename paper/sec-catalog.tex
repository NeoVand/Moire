...
residual into a distance. This section is the list. It is short, and its shortness
is the point --- thirteen families, of which nine are one line of algebra each.
The two that are not, and the reason they are not, set up the rest of the paper.
Appendix~\ref{app:catalog} draws all thirteen, each alone and then beating against
a perturbed copy of itself (Figure~\ref{fig:plate}), which is the fastest way to
see what the algebra below buys.
...
Table~\ref{tab:catalog} gives both for all nine. Every entry is $O(1)$, branch-free
apart from the free-side $N$-gon, and calls no transcendental function except where
the geometry genuinely contains an angle.

\begin{table*}[t]
  \centering
  \caption{\figlead{Nine families as phase functions.} Each row is a complete
  specification: substitute $\ph$ and $\lvert\nabla\ph\rvert$ into
  \eqref{eq:closedpair} and the layer renders. $\omega=2\pi f/32$ so that
  ...
  \begin{tabular}{@{}llll@{}}
    ...
    Concentric $N$-gon
      ...
      & $1$
      & $\theta_p=\mathrm{atan2}(y,x)$; the only $\mathrm{atan2}$ \\
    Wave
...
Three remarks on the table, each of which cost us a bug.
...
stroke by exactly the factor the family is steep by. We had shipped that bug in
...
raster pipeline hides in the resampling and a field pipeline puts in your face on
the first frame.
...
\subsection{Two families that are not phase functions}
...
the lines all meet at a point, so any such function would have to take every value
at the origin. What there is, is a distance,
\begin{equation}
...
$\lfloor \theta_p N/\pi\rfloor$ --- which is what the fringe law needs. The second
...
and every author reaches for the same fix. Because the sector index is piecewise
...
rounding on $(u,v,-u-v)$, which is exact and branch-light --- and the layer offers
...
The consequence for the theory is worth stating plainly. Theorem~\ref{thm:fringe}
...
exactly the classical situation, where a $2$-D screen superposition has a fringe
...
formulation does not remove that bookkeeping. It localises it: each component is a
scalar field we can evaluate, difference, and map.

\subsection{And one that has no index field at all}
...
what most op-art actually looks like. But $\ph$ is gone, and not for a technical
reason: each member now lives in its own frame, so there is no function of $p$
alone whose level sets are these curves. The nearest index must be searched for.
