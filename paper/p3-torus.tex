...
recipe; a recipe with an entry beyond one that shows is called a
\emph{station}, because it locks the two families at a rational ratio of
their pitches, and its \emph{order} is the sum of its entries' sizes.
...
distance, or per second for a family in time. The rate of a combined count is $\nabla(k\cdot\cnt)=\sum_ik_i\nabla\cnt_i$.
...
\begin{table}[t]
  ...
  \begin{tabular}{@{}lll@{}}
    ...
    $g_i$, $\hat g_i(n)$ & a family's profile, and its $n$-th harmonic's strength & \S\ref{sec:whosees} \\
    $\mu_{a,b}$ & the priced merit: $\het$ charged $|ab|$ & Definition~\ref{def:merit} \\
    $h_n/k_n$, $x_{n+1}$ & a convergent and a complete quotient of a ratio & \S\ref{sec:selection} \\
    $F(x,c)$ & the incidence: ``$x$ is on member $c$'' & \S\ref{sec:counting} \\
    $q$ & a defect's charge: the whole number a count gains round it & \S\ref{sec:counting} \\
    $s$, $\delta$, $\theta$, $\gamma$ & a walking family's steps in size, position and angle, and its shape's gauge & \S\ref{sec:counting} \\
    \bottomrule
  \end{tabular}
  \caption{\figlead{Every symbol in the paper}, and where it is introduced.
  Everything below $\het$ is used from \S\ref{sec:pooling} on.}
  \label{tab:symbols}
\end{table}
...
where $\het<\tfrac14$ (for recipes beyond first order, the priced merit of
\S\ref{sec:selection}): the bands you see in the ordinary view sit exactly
