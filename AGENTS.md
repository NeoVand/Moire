...
- Ratio view is the heterodyne ratio of the two topmost visible scalar-index layers (no lattices; radial fan counts), picked on the CPU into the `ratioA`/`ratioB` uniforms — never a rebuild. Fractional index is the phase sample's signed residual over the gap toward `min(rUp, rDown)` (smaller residual is always the next member up); `dFdx/dFdy` per layer, integer wraps rounded out of the per-pixel deltas. A visible moiré is an integer character `kA·ξA + kB·ξB`, so η is the MIN over the primitive characters with |k| ≤ 2 (`(1,∓1)` the classical difference/sum moirés, `(2,∓1)`/`(1,∓2)` the second-order rational-pitch beats) of `|kA∇ξA + kB∇ξB| / (|kA∇ξA − kB∇ξB|/2)`, clamped to [0,1]. The winning character also sets the envelope's per-layer sweep rates `(|kB|, ∓kA)` — the sweep runs orthogonal to the kept character, so second-order beats survive the average (a |rate| of 2 wraps the slide offset back into the trio's half-period; exact for phase families). Dark ≤ threshold (default 1/4) is the fringe regime; `ratio` widens the solver guard like `sweep`. Fewer than two eligible layers renders the plain composite.
- The envelope reuses that η twice: `envMask` fades the view to its pivot past the threshold (band follows the `ratioThreshold` uniform; outside the regime the mean is a faithful but carrier-fine Φ(D) that reads as a failed average — mask defaults OFF, slider in the panel), and the ranked pair advances at the winning character's rates instead of the plain diagonal (diagonal = `(1,1)` preserves φ₁−φ₂; `(1,−1)` recovers the sum moiré; `(1,±2)`/`(2,±1)` hold the second-order beats). Pair uniforms stay warm during any enveloped frame.
