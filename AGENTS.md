...
- The envelope reuses that η twice: `envMask` fades the view to its pivot where η > 1/4 (outside the regime the mean is a faithful but carrier-fine Φ(D) that reads as a failed average — mask defaults on, slider in the panel), and where the SUM beat is the slower one the ranked second layer sweeps backwards (the diagonal preserves φ₁−φ₂ and averages φ₁+φ₂ away; flipping recovers the sum moiré). Pair uniforms stay warm during any enveloped frame.
