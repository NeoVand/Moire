...
float multRe(float phi0, float2 b, float3 q, float S) {
  ...
  float modu = rsqrt(sqrt((1.0 + S * S * l1 * l1) * (1.0 + S * S * l2 * l2)));
  float ph = 0.5 * atan2(S * (l1 + l2), 1.0 - S * S * l1 * l2); // atan a + atan b = atan2(a + b, 1 - a b)
...
float2 checkerSpectral(Jets J, float S) {
  ...
  int nCount = int(min(nMax, 4096.0)) + 1; // the half lattice; the conjugate half is folded in
  for (int iN = 0; iN < nCount; iN++) {
    ...
    for (int iM = 0; iM < mCount; iM++) {
      ...
      if (ok && half) {
        ...
        float lamT = sqrt(qq.x * qq.x + 2.0 * qq.y * qq.y + qq.z * qq.z);
        float reach = OURS_TAU * 1.6 * sqrt(1.0 + S * S * lamT * lamT);
...
float bvnu(float h, float k, float r) {
  ...
  { float sn = sin(0.9801449282 * asr); bvn += 0.1012285363 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { float sn = sin(0.8983332387 * asr); bvn += 0.2223810345 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { float sn = sin(0.7627662050 * asr); bvn += 0.3137066459 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { float sn = sin(0.5917173212 * asr); bvn += 0.3626837834 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { float sn = sin(0.4082826788 * asr); bvn += 0.3626837834 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { float sn = sin(0.2372337950 * asr); bvn += 0.3137066459 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { float sn = sin(0.1016667613 * asr); bvn += 0.2223810345 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  { float sn = sin(0.01985507175 * asr); bvn += 0.1012285363 * exp((sn * hk - hs) / (1.0 - sn * sn)); }
  } else {
